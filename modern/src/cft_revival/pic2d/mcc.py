...
class XenonCrossSections:
    ...
    def __post_init__(self) -> None:
        ...
        if identifiers != PROCESS_ORDER:
            raise PIC2DValidationError(f"processes must be exactly {PROCESS_ORDER} in order, got {identifiers}")

    @classmethod
    def from_file(cls, path: Path = DEFAULT_CROSS_SECTION_PATH) -> "XenonCrossSections":
        raw = path.read_bytes()
        document = json.loads(raw.decode("utf-8"))
        if document.get("schema") != "cft.pic2d.xenon-cross-sections.v1":
...
class UniformSigmaTable:
    ...
    energy_max_ev: float
    table_m2: np.ndarray  # shape (3, n_points)
    ...
    def build(cls, cross_sections: XenonCrossSections, *, energy_step_ev: float = 0.05, energy_max_ev: float = 2000.0) -> "UniformSigmaTable":
        ...
        return cls(float(energy_step_ev), float(grid[-1]), table, thresholds)  # type: ignore[arg-type]
    ...
    def lookup(self, energy_ev: np.ndarray) -> np.ndarray:
        """Piecewise-linear lookup; returns shape (3, N).  Clamps above the grid."""
...
class NullCollisionMCC:
    ...
    def apply(self, electrons: ParticleArrays, dt_s: float, rng: np.random.Generator) -> MCCResult:
        ...
        excitation = candidate & ~elastic & (selector < cumulative[1])
        ionization = candidate & ~elastic & ~excitation & (selector < cumulative[2])
        ...
        # Excitation: lose the threshold, isotropic.
        if np.any(excitation):
            remaining = np.maximum(energy[excitation] - thresholds[1], 0.0)
            nvx, nvy, nvz = isotropic_velocity(electron_speed_from_energy(remaining), u[2][excitation], u[3][excitation])
            vx[excitation], vy[excitation], vz[excitation] = nvx, nvy, nvz
        new_electrons = ParticleArrays.empty()
        new_ions = ParticleArrays.empty()
        ion_r = np.zeros(0)
        ion_z = np.zeros(0)
        if np.any(ionization):
            available = np.maximum(energy[ionization] - thresholds[2], 0.0)
