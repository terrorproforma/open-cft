...
``modern/experiments/pic2d_cft_snapshot_v2/results`` (model v1.1) and keeps the
v1 cases (``pic2d_cft_snapshot_v1``, which the runtime stability gate stopped
fail-closed at ~50 ns) in a development-history panel.  No timestamps or runtime
...
PROTOCOL = MODERN / "experiments" / "pic2d_cft_snapshot_v2" / "protocol.json"
HISTORY_RESULTS = MODERN / "experiments" / "pic2d_cft_snapshot_v1" / "results"
HISTORY_PROTOCOL = MODERN / "experiments" / "pic2d_cft_snapshot_v1" / "protocol.json"
DEFAULT_OUTPUT = Path(__file__).with_name("pic2d-cft-snapshot.html")
SCHEMA = "cft-pic2d-cft-snapshot-visualization/0.2.0"
STOP_REASONS = {
    "target_steps_reached", "wall_clock_budget_reached", "runtime_stability_gate_stopped_run", "plateau_reached_after_min_steps",
}
...
SERIES_KEYS = (
    ...
    "field_energy_j", "total_energy_j", "interval_residual_j", "interval_sources_j", "interval_electrode_work_j",
    "peak_omega_pe_dt",
...
def build_history(results: Path = HISTORY_RESULTS, protocol_path: Path = HISTORY_PROTOCOL) -> dict[str, Any]:
    """Hash-verified digest of the v1 cases (fail-closed development history; no maps)."""

    manifest_path = results / "manifest.json"
    if not manifest_path.is_file():
        raise ValueError("v1 snapshot manifest is missing")
    ...
    if _file_sha256(protocol_path) != manifest["protocol_sha256"]:
        raise ValueError("v1 protocol file differs from the hash recorded in its manifest")
    cases = []
    for case, entry in manifest["cases"].items():
        ...
        if _verify_sidecar(summary_path) != entry["summary_sha256"]:
            raise ValueError(f"{case}: v1 summary SHA-256 differs from manifest")
        ...
        cases.append({
            ...
            "final_discharge_a": summary["final_series"]["currents_a"]["discharge_a"],
            "final_peak_omega_pe_dt": summary["final_series"].get("max_omega_pe_dt"),
            "summary_sha256": entry["summary_sha256"],
        })
    diagnosis_path = results / "diagnosis.json"
    diagnosis: dict[str, Any] | None = None
    if diagnosis_path.is_file():
        _verify_sidecar(diagnosis_path)
        raw = read_canonical_json(diagnosis_path)
        # diagnosis.json: equilibrium_0d.at_observed_t_e_and_i_d.{rows, assumptions}
        equilibrium = raw.get("equilibrium_0d", {}).get("at_observed_t_e_and_i_d", raw.get("equilibrium", {}))
        diagnosis = {
            "equilibrium_rows": [
                {k: row[k] for k in ("n_g_per_m3", "amplification_source_over_loss", "particle_balance_t_e_ev", "n_eq_power_balance_per_m3", "lambda_d_m_at_n_eq", "omega_pe_rad_per_s_at_n_eq") if k in row}
                for row in equilibrium.get("rows", [])
            ],
            "assumptions": {k: v for k, v in equilibrium.get("assumptions", {}).items() if k != "note"},
        }
    return {
        ...
        "dt_s": protocol["numerics"]["dt_s"],
        "cases": cases,
        "diagnosis": diagnosis,
        "lesson": (
            "v1 ran 0.1 A of injected electrons into a static 5e20 m^-3 xenon background at 300 V: the 0-D source/loss "
            "amplification was 12 per injected electron, the discharge had no saturation channel before ions reached the "
            "boundaries (~1 us), and the on-axis peak density passed omega_pe*dt = 0.2 after ~50 ns, so the runtime gate "
            "stopped every case fail-closed. v1.1 rescaled the operating point (3 mA, 1e20 m^-3, dt = 1.5 ps) so the 0-D "
            "equilibrium density is resolvable on the fine grid; the v2 cases above show that the kinetic discharge still "
            "grows past that 0-D lower bound within one ion transit time (the 0-D loss is an unmagnetised Bohm upper bound)."
        ),
    }


def build_payload(results: Path = RESULTS, protocol_path: Path = PROTOCOL, history_results: Path | None = HISTORY_RESULTS,
                  history_protocol: Path | None = HISTORY_PROTOCOL) -> dict[str, Any]:
    manifest_path = results / "manifest.json"
    if not manifest_path.is_file():
        ...
        cases.append(
            {
                ...
                "ms_per_step": float(f"{summary['ms_per_step']:.4g}") if "ms_per_step" in summary else None,
    ...
    history = None
    if history_results is not None and history_protocol is not None and (history_results / "manifest.json").is_file():
        history = build_history(history_results, history_protocol)
    payload = {
        "schema": SCHEMA,
        "experiment_id": manifest["experiment_id"],
        "model_version": manifest.get("model_version", protocol.get("model_version")),
        "status": manifest["status"],
        "claim_boundary": manifest["claim_boundary"],
        "claim_statement": (
            "Development/screening PIC-MCC snapshot (model v1.1). Not preregistered. Not validated against any experiment. "
            "Not a thruster performance prediction. The operating point (3 mA injection, n_g = 1e20 m^-3; not a real "
            "0.1-1 A discharge) was chosen from a 0-D estimate so the equilibrium density would be resolvable; the kinetic "
            "runs grew past that budget within one ion transit time (window peak n_e 3.7-5.9 x n_max, cells 3-6 lambda_D "
            "at the end, no plateau), and two coarse cases were stopped fail-closed by the omega_pe dt gate. That growth, "
            "not the current or density values, is the screening result. Numerics verified by the tests in "
            "modern/tests/pic2d; physics simplified as listed."
        ),
        ...
        "budget": manifest.get("budget_v1_1", protocol.get("budget_v1_1")),
...
def validate_payload(payload: Mapping[str, Any]) -> None:
    required = {
        "schema", "experiment_id", "model_version", "status", "claim_boundary", "claim_statement", "simplifications", "manifest",
        "protocol", "budget", "convergence", "cases", "history",
    }
    ...
    if payload["history"] is not None:
        for case in payload["history"]["cases"]:
            if case["stop_reason"] not in STOP_REASONS:
                raise ValueError(f"history {case['id']}: unknown stop reason")
            if not isinstance(case["summary_sha256"], str) or len(case["summary_sha256"]) != 64:
                raise ValueError(f"history {case['id']}: summary_sha256 must be a SHA-256")
...
<body>
<header><div class="eyebrow">PIC-MCC · axisymmetric (r,z) · development snapshot v2 · model v1.1</div><h1>Divergent-exit CFT channel: kinetic snapshot v2 — growth past the resolvability budget</h1>
...
<section class="panel" style="margin:1rem 0"><h2>v1.1 resolvability budget (a priori)</h2><div id="budget"></div></section>
<section class="panel" style="margin:1rem 0"><h2>Fail-closed development history (snapshot v1)</h2><div id="history"></div></section>
<section class="panel" style="margin:1rem 0"><h2>Simplifications (model v1.1) and identity</h2><div id="identity"></div></section>
...
const B=DATA.budget;if(B){const rows=Object.entries(B.cases||{}).map(([k,v])=>`<tr><td>${k}</td><td>${fmt(v.dr_m*1e6,3)} × ${fmt(v.dz_m*1e6,3)}</td><td>${fmt(v.max_cell_over_lambda_d,3)}</td><td>${v.debye_ratio_limit}</td><td>${sci(v.macro_weight,2)}</td></tr>`).join("");$("budget").innerHTML=`<div class="kv" style="grid-template-columns:repeat(auto-fit,minmax(220px,1fr))"><span>n_max (design ceiling)</span><span>${sci(B.n_max_per_m3,2)} m⁻³</span><span>T_e for λ_D</span><span>${fmt(B.t_e_ev,2)} eV</span><span>λ_D,min</span><span>${fmt(B.lambda_d_min_m*1e6,3)} µm</span><span>Δt</span><span>${sci(B.dt_s,2)} s</span><span>ω_pe Δt at n_max</span><span>${fmt(B.omega_pe_dt,3)}</span><span>Ω_ce Δt at |B|max</span><span>${fmt(B.omega_ce_dt,3)}</span><span>ion subcycle k</span><span>${B.ion_subcycle}</span><span>ω_pi k Δt</span><span>${sci(B.omega_pi_k_dt,2)}</span></div><table style="width:100%;border-collapse:collapse;font-variant-numeric:tabular-nums;margin-top:.6rem" aria-label="Resolution budget per case"><thead><tr><th style="text-align:left">case</th><th>Δr × Δz (µm)</th><th>max cell / λ_D,min</th><th>gate limit</th><th>macro weight</th></tr></thead><tbody>${rows}</tbody></table><p class="small">The coarse pair exceeds the 2× cell/Debye rule by design (it is the under-resolved member of the convergence pair; its gate limit is 3.1×). Operating point: ${DATA.protocol.operating_point.electron_injection_current_a*1e3} mA injection, n_g = ${sci(DATA.protocol.operating_point.neutral_density_per_m3,2)} m⁻³, ${DATA.protocol.operating_point.anode_potential_v} V. Stopping rule: ${typeof DATA.protocol.stopping_rule==="string"?DATA.protocol.stopping_rule:DATA.protocol.stopping_rule.description+" Plateau: "+DATA.protocol.stopping_rule.plateau}</p>`}
const H=DATA.history;if(H){const hrows=H.cases.map(h=>`<tr><td>${h.id}</td><td>${h.grid}</td><td>${sci(h.macro_weight,2)}</td><td>${h.steps_completed}</td><td>${fmt(h.simulated_time_s*1e9,3)}</td><td>${sci(h.n_e_peak_per_m3,3)}</td><td>${fmt(h.final_discharge_a*1e3,3)}</td><td>${h.stop_reason.replaceAll("_"," ")}</td></tr>`).join("");const eq=H.diagnosis&&H.diagnosis.equilibrium_rows?`<table style="width:100%;border-collapse:collapse;font-variant-numeric:tabular-nums;margin-top:.6rem" aria-label="0-D equilibrium estimate"><thead><tr><th style="text-align:left">n_g (m⁻³)</th><th>source/loss gain at v1 T_e</th><th>T_e,balance (eV)</th><th>n_eq power balance (m⁻³)</th><th>λ_D (µm)</th><th>ω_pe (s⁻¹)</th></tr></thead><tbody>${H.diagnosis.equilibrium_rows.map(r=>`<tr><td>${sci(r.n_g_per_m3,2)}</td><td>${fmt(r.amplification_source_over_loss,3)}</td><td>${fmt(r.particle_balance_t_e_ev,3)}</td><td>${sci(r.n_eq_power_balance_per_m3,3)}</td><td>${fmt(r.lambda_d_m_at_n_eq*1e6,3)}</td><td>${sci(r.omega_pe_rad_per_s_at_n_eq,3)}</td></tr>`).join("")}</tbody></table>`:"";$("history").innerHTML=`<p class="small">${H.lesson}</p><p class="small">v1 operating point: ${H.operating_point.electron_injection_current_a} A injection, n_g = ${sci(H.operating_point.neutral_density_per_m3,2)} m⁻³, Δt = ${sci(H.dt_s,2)} s. Every v1 case was stopped by the runtime ω_pe Δt gate; these are hash-verified from <code>${H.experiment_id}</code> (manifest <code>${H.manifest_sha256}</code>).</p><table style="width:100%;border-collapse:collapse;font-variant-numeric:tabular-nums" aria-label="v1 fail-closed cases"><thead><tr><th style="text-align:left">case</th><th>grid</th><th>weight</th><th>steps</th><th>t (ns)</th><th>window peak n_e</th><th>final I_d (mA)</th><th>stop</th></tr></thead><tbody>${hrows}</tbody></table>${eq}`}
