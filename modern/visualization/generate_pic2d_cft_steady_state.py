...
<section class="grid">
<div class="panel"><h2 id="mapTitle">Plateau-window map</h2><div class="canvas-wrap"><canvas id="field" tabindex="0" role="img" aria-label="Interactive (r,z) heatmap of the selected plateau-window quantity"></canvas><div id="tip" class="tip" role="status" aria-live="polite"></div></div><p class="small">Canvas raster of the node grid (radial-major). White: dielectric/outside the plasma cell mask. Straight bore wall at r = 2 mm is exact; the cone is a one-cell stair-step. Anode at z = 0 (fixed potential), exit plane at z = 24 mm (0 V reference). Dashed lines: cusp planes.</p></div>
