...
def test_see_off_reproduces_the_v2_0_6_identity_ledger_and_tallies():
    ...
    # column (4 = stream id 5), the v2.4.0 Coulomb column (5 = stream id 6) and the v2.5.0 spatial-neutral column (6 = stream id 7):
    # the table row is read per column, so a wider row leaves every earlier column's seeds as they were
    try:
        ...
        return
    assert SEED_STREAMS == 7 and SEED_STREAM_IDS == (1, 2, 3, 4, 5, 6, 7) and SEE_STREAM == 4
