...
from experiments.wall_loss_geometry_surrogate_v1 import experiment, run
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda r: r["development"].__setitem__("candidates_fitted", ["pkg-exactgp-logit"]), "all_candidates_fitted"),
...
def test_bind_preregistration_constants() -> None:
    assert run.SUBJECT == "preregister wall-loss geometry surrogate v1"
    assert run.REMOTE_BRANCH == "origin/exp/wall-loss-geometry-surrogate-v1"
    assert protocol()["execution"]["git_common_lock"] == "wall-loss-geometry-surrogate-v1.execution.lock"
