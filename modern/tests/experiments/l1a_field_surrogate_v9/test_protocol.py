...
from experiments.l1a_field_surrogate_v9.experiment import (
    SharedKernelGP,
    WeightedPOD,
    model_features,
    qoi_inverse,
...
from experiments.l1a_field_surrogate_v9.protocol import (
    DEPENDENCY_LOCK,
    GEOMETRY_PREFLIGHT,
    PARTITIONS,
    PROTOCOL,
...
from experiments.l1a_field_surrogate_v9.run import (
...
def test_audited_model_families_and_transforms_are_frozen() -> None:
    ...
    assert PROTOCOL["models"]["field_families"] == [
        "observed_coarse_stagewise_lossless_physical_residual_ard_matern"
    ]
...
def test_ard_matern_fits_constant_mean_without_unscaled_fallback() -> None:
    ...
    assert "exp(0)" not in inspect.getsource(
        __import__(
            "experiments.l1a_field_surrogate_v9.run",
...
@pytest.mark.parametrize(
    "status",
    (
        " M modern/experiments/l1a_field_surrogate_v9/predeclaration.json",
        "M  modern/experiments/l1a_field_surrogate_v9/run.py",
        "?? foreign-untracked.txt",
        "?? modern/experiments/l1a_field_surrogate_v9/results-sibling/file.json",
    ),
)
