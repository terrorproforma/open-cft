...
from experiments.l1a_field_surrogate_v7.experiment import (
    SharedKernelGP,
    model_features,
    qoi_inverse,
    qoi_transform,
)
from experiments.l1a_field_surrogate_v7.protocol import (
    DEPENDENCY_LOCK,
    GEOMETRY_PREFLIGHT,
    PARTITIONS,
    PROTOCOL,
...
from experiments.l1a_field_surrogate_v7.run import (
...
def test_sealed_preflight_is_complete_solver_free_and_runtime_owned() -> None:
    ...
    assert geometry["raw_count"] == 768
    assert geometry["frozen_hash_failure_count"] == 0
    assert len(partitions["frozen_raw_indices"]) == 368
...
def test_audited_model_families_and_transforms_are_frozen() -> None:
    assert PROTOCOL["sampling"]["high_budgets"] == [128, 176, 224]
    assert PROTOCOL["models"]["scalar_families"] == [
        "observed_coarse_standardized_log_ratio_ard_matern"
    ]
    assert PROTOCOL["models"]["field_families"] == [
        "observed_coarse_stagewise_residual_aligned_pod_ard_matern"
    ]
    coarse = {name: 1.2 for name in PROTOCOL["models"]["qois"]}
    ordinary = model_features([0.1] * 11, coarse, "field_energy_j")
    phased = model_features([0.1] * 11, coarse, "source_representation_error")
    assert ordinary.shape == (12,)
    assert phased.shape == (24,)
...
def test_ard_matern_fits_constant_mean_without_unscaled_fallback() -> None:
    ...
    assert "exp(0)" not in inspect.getsource(
        __import__(
            "experiments.l1a_field_surrogate_v7.run",
...
@pytest.mark.parametrize(
    "status",
    (
        " M modern/experiments/l1a_field_surrogate_v7/predeclaration.json",
        "M  modern/experiments/l1a_field_surrogate_v7/run.py",
        "?? foreign-untracked.txt",
        "?? modern/experiments/l1a_field_surrogate_v7/results-sibling/file.json",
    ),
)
