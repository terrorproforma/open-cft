...
from experiments.mdo_l0_campaign_v1 import experiment, run
...
def test_shakedown_design_is_disjoint_from_the_evidentiary_design() -> None:
    ...
    assert report["seed_overlap"] == []
    assert report["run_id_overlap"] == []
...
def _require_ml_runtime() -> None:
    """The gate binds the ML runtime versions; without them it fails closed by design."""
...
def test_shakedown_record_passes_the_gate_or_is_bound_to_the_recorded_bundle() -> None:
    ...
    if _recorded_lifecycle():
        # After the single execution the live packages may legitimately drift; the
        # recorded contract lives in the bundle.  The bundle's shakedown copy must be
        # byte-identical to the frozen file and the authorities must bind it.
...
def test_bind_preregistration_requires_the_exact_subject() -> None:
    assert run.SUBJECT == "preregister MDO L0 campaign v1"
    assert run.REMOTE_BRANCH == "origin/exp/mdo-l0-campaign-v1"
    assert protocol()["execution"]["git_common_lock"] == "mdo-l0-campaign-v1.execution.lock"


def test_frozen_authorities_bind_protocol_source_and_shakedown_when_present() -> None:
    ...
    assert authorities["sample_sha256"] == value["uncertain_inputs"]["sample"]["sha256"]
    if not _recorded_lifecycle():
        # Before execution the live source tree must still match the frozen hash.
