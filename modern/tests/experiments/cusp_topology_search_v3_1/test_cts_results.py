"""Lifecycle-aware consistency of the sealed v3.1 bundle (skips before execution)."""
...
from experiments.cusp_topology_search_v3_1 import catalogue as C
...
def test_design_records_reproduce_the_dataset_rows_and_the_catalogue(dataset: dict) -> None:
    ...
    for row in dataset["designs"]:
        ...
        if record["set_id"] == "characterization_v1":
            roots = record["reference"]["v1_primary_axis_roots"]
            assert all(any(method in ("axis_sign_change", "axis_grid") for method in root["methods"]) for root in roots)
...
def test_held_out_and_p2_consistency_are_recorded(dataset: dict) -> None:
    ...
    assert held_out["sweep_v2"]["reference_null_count"] == held_out["sweep_v2"]["observed_null_count"]
