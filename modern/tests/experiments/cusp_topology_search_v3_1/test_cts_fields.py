...
def test_v1_axis_reference_keeps_multi_member_clusters_that_v3_dropped() -> None:
    """The v3 defect case: s05-p0-r0-neg has four sealed channel axis clusters, two with r != 0."""

    recorded = next(case for case in F.v1_dataset()["cases"] if case["case_id"] == "topology-s05-p0-r0-neg")
    roots = F.v1_axis_reference(recorded)
    channel = [root for root in roots if root["zone"] in ("plasma_channel", "channel_axial_margin")]
    assert len(channel) == 4
    assert sum(root["centroid_r_m"] != 0.0 for root in channel) == 2
    assert all(root["classification"] == "X" for root in channel)
    assert all(any(method in F.V1_AXIS_DETECTION_METHODS for method in root["methods"]) for root in roots)
    assert [round(root["z_m"], 6) for root in channel] == [0.007635, 0.011209, 0.014786, 0.018379]
    # Over the whole sealed dataset: 206 axis clusters, 26 with a nonzero centroid.
    clusters = [root for case in F.v1_dataset()["cases"] for root in F.v1_axis_reference(case)]
    assert len(clusters) == 206 and sum(root["centroid_r_m"] != 0.0 for root in clusters) == 26
    assert max(abs(root["centroid_r_m"]) for root in clusters) < 1.0e-7
