...
from experiments.l1a_field_surrogate_v8.experiment import (
    SharedKernelGP,
    model_features,
    qoi_inverse,
    qoi_transform,
)
from experiments.l1a_field_surrogate_v8.protocol import (
    DEPENDENCY_LOCK,
    GEOMETRY_PREFLIGHT,
    PARTITIONS,
    PROTOCOL,
...
from experiments.l1a_field_surrogate_v8.run import (
...
def test_ard_matern_fits_constant_mean_without_unscaled_fallback() -> None:
    ...
    assert "exp(0)" not in inspect.getsource(
        __import__(
            "experiments.l1a_field_surrogate_v8.run",
...
@pytest.mark.parametrize(
    "status",
    (
        " M modern/experiments/l1a_field_surrogate_v8/predeclaration.json",
        "M  modern/experiments/l1a_field_surrogate_v8/run.py",
        "?? foreign-untracked.txt",
        "?? modern/experiments/l1a_field_surrogate_v8/results-sibling/file.json",
    ),
)
