"""Synthetic detached identity and lock tests for v8."""
...
def _repository(tmp_path: Path) -> Path:
    ...
    _git(source, "config", "user.name", "V8 Test")
    _git(source, "config", "user.email", "v8@example.invalid")
    ...
    for relative, content in (
        ("modern/experiments/l0_surrogate_v8/protocol.py", "V = 8\n"),
        (
            "modern/experiments/l0_surrogate_v8/dependency-manifest.json",
            json.dumps(manifest),
        ),
        (
            "modern/tests/experiments/l0_surrogate_v8/test_protocol.py",
    ...
    _git(source, "add", "modern/experiments/l0_surrogate_v8")
    _git(source, "add", "modern/tests/experiments/l0_surrogate_v8")
...
def test_clean_detached_binding_and_dirty_rejection(tmp_path: Path) -> None:
    ...
    manifest = detached / "modern/experiments/l0_surrogate_v8/dependency-manifest.json"
...
def test_exclusive_lock_is_atomic_and_retained(tmp_path: Path) -> None:
    detached = _repository(tmp_path)
    commit = _git(detached, "rev-parse", "HEAD")
    first = acquire_lock(detached, commit)
