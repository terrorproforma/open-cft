"""Production geometry, field transfer, GP and weighted-POD paths for v4."""
...
def _v3_role_rows() -> set[tuple[float, ...]]:
...
def raw_designs() -> tuple[Any, ...]:
    ...
    prior.update(_v3_role_rows())
    if prior.intersection(tuple(item.values) for item in designs):
        raise RuntimeError("v4 coordinates overlap v1/v2/v3 role evidence")
    return designs
...
def construct_geometry(design: Any, raw_index: int) -> tuple[Any, dict[str, Any]]:
    ...
    case = replace(
        case,
        case_id=f"l1a-fs-v4-{raw_index:03d}",
        config_sha256=config_hash,
        case_sha256=case_hash,
    )
    return case, {
        **record,
        "v4_case_id": case.case_id,
        "v4_config_sha256": config_hash,
        "v4_case_sha256": case_hash,
    }
