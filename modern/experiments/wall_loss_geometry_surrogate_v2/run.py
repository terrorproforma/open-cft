...
Lifecycle (from ``modern/`` with the ``.venv-sota`` interpreter, cpu only)::
    ...
    python -m experiments.wall_loss_geometry_surrogate_v2.run shakedown   # any HEAD, BEFORE prepare
    python -m experiments.wall_loss_geometry_surrogate_v2.run prepare     # refuses without a valid shakedown.json
...
SUBJECT = "preregister wall-loss geometry surrogate v2"
REMOTE_BRANCH = "origin/exp/wall-loss-geometry-surrogate-v2"
COMMAND = "python -m experiments.wall_loss_geometry_surrogate_v2.run execute"
SHAKEDOWN_COMMAND = "python -m experiments.wall_loss_geometry_surrogate_v2.run shakedown"
DEVICE = "cpu-float64-botorch+sklearn-gbt;8-threads;gpu-not-used"
FROZEN_OUTPUTS = (AUTHORITIES_PATH, PARTITIONS_PATH)
EXPERIMENT_PREFIX = "modern/experiments/wall_loss_geometry_surrogate_v2/"
TESTS_PREFIX = "modern/tests/experiments/wall_loss_geometry_surrogate_v2/"
...
def shakedown() -> dict[str, Any]:
    ...
    temp_root = Path(tempfile.gettempdir()) / f"wlgs-v2-shakedown-{git['head'][:12]}-{stamp}-{token}"
...
def prepare() -> dict[str, Any]:
    ...
    if semantic_sha256(partition) != value["v1_binding"]["partitions_semantic_sha256"]:
        raise RuntimeError("prepare refused: the evidentiary partition does not equal v1's committed partitions.json")
    _write(PARTITIONS_PATH, partition)
    if PARTITIONS_PATH.read_bytes() != (REPOSITORY / value["v1_binding"]["files"]["partitions"]["path"]).read_bytes():
        raise RuntimeError("prepare refused: partitions.json is not byte-identical to v1's")
    authorities = {
        "schema_version": schema("authorities"),
        "classification": CLASSIFICATION,
        "inherits_v1_partition": True,
        "v1_partitions_semantic_sha256": value["v1_binding"]["partitions_semantic_sha256"],
        "v1_result_commit": v1_binding["result_commit"],
        "v1_file_sha256": v1_binding["file_sha256"],
        "protocol_semantic_sha256": semantic_sha256(value),
...
def execute() -> dict[str, Any]:
    ...
    value = protocol()
    cache_root = Path(tempfile.gettempdir()) / f"wlgs-v2-{commit[:12]}-working-cache"
    runtime = ExperimentRuntime(
