"""Sole launcher for the clean-detached v7 validation attempt."""
...
COMMAND = "python -m experiments.cft_wall_cusp_validation_v7.run"
...
def main() -> None:
    ...
    promoted = outcome.state.value == "accepted_result"
    print(
        f"state={outcome.state.value} "
        f"promotion={'true' if promoted else 'false'} "
        f"gates={'passed' if promoted else 'failed'} "
