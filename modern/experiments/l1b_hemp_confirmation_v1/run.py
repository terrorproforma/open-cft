...
Lifecycle (from ``modern/``)::
    ...
    python -m experiments.l1b_hemp_confirmation_v1.run shakedown  # any HEAD, BEFORE prepare
    python -m experiments.l1b_hemp_confirmation_v1.run prepare    # refuses without a valid shakedown.json
...
SUBJECT = "preregister L1b HEMP confirmation v1"
REMOTE_BRANCH = "origin/exp/l1b-hemp-confirmation-v1"
COMMAND = "python -m experiments.l1b_hemp_confirmation_v1.run execute"
SHAKEDOWN_COMMAND = "python -m experiments.l1b_hemp_confirmation_v1.run shakedown"
...
EXPERIMENT_PREFIX = "modern/experiments/l1b_hemp_confirmation_v1/"
TESTS_PREFIX = "modern/tests/experiments/l1b_hemp_confirmation_v1/"
...
def shakedown() -> dict[str, Any]:
    ...
    temp_root = Path(tempfile.gettempdir()) / f"l1b-hc-v1-shakedown-{git['head'][:12]}-{stamp}-{token}"
...
def execute() -> dict[str, Any]:
    ...
    value = protocol()
    cache_root = Path(tempfile.gettempdir()) / f"l1b-hc-v1-{commit[:12]}-working-cache"
    runtime = ExperimentRuntime(
