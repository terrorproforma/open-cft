...
def solve_two_level(geometry: AxisymmetricCFTGeometry, value: Mapping[str, Any], budget: RamBudget) -> P2Solution:
    ...
    for level in range(level_count):
        ...
        if float(quality["minimum_angle_deg"]) < float(mesh_declaration["reject_below_angle_deg"]):
            raise ValueError(f"level {level} mesh violates the minimum-angle rejection gate")
        ...
        levels.append(
            P2Level(
                ...
                len(mesh.triangles),
                _robin_edges(mesh),
                {key: (float(item) if isinstance(item, (float, np.floating)) else int(item)) for key, item in quality.items()},
                allocation,
                solve_seconds,
                rss,
                adaptivity,
