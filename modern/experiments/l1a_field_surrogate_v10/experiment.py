"""Localized lossless physical-residual surrogate paths for v10."""
...
def raw_designs() -> tuple[Any, ...]:
    ...
    prior.update(_prior_role_rows(8))
    prior.update(_prior_role_rows(9))
    if prior.intersection(tuple(item.values) for item in designs):
        raise RuntimeError("v10 coordinates overlap v1-v9 evidence")
    return designs
...
def construct_geometry(design: Any, raw_index: int) -> tuple[Any, dict[str, Any]]:
    ...
    case = replace(
        case,
        case_id=f"l1a-fs-v10-{raw_index:04d}",
        config_sha256=config_hash,
        case_sha256=case_hash,
    )
    return case, {
        **record,
        "v10_case_id": case.case_id,
        "v10_config_sha256": config_hash,
        "v10_case_sha256": case_hash,
    }
...
class WeightedPOD:
    """Stage-local lossless-primary multi-view residual basis."""
    ...
    def _augment(cls, snapshots: np.ndarray, stage_count: int) -> np.ndarray:
        ...
        channels = [
            values,
            values * np.sqrt(weights)[None, :] / energy_scale,
        ]
        nr, nz = HIGH_DOMAIN.shape
        z = np.linspace(0.0, 1.0, nz)
        centres = np.linspace(0.0, 1.0, stage_count + 2)
        width = 1.75 / max(stage_count + 1, 1)
        for centre in centres:
            distance = np.abs(z - centre) / width
            taper = np.where(
                distance < 1.0,
                0.5 * (1.0 + np.cos(math.pi * distance)),
                0.0,
            )
            full_taper = np.tile(taper, nr)
            channels.append(
                values
                * np.concatenate((full_taper, full_taper))[None, :]
                / math.sqrt(max(float(np.sum(taper)), 1e-12))
            )
        axis = values[:, nr * nz : nr * nz + nz]
        channels.extend(
            (
                2.0 * axis / math.sqrt(nz),
                np.gradient(axis, axis=1) / math.sqrt(nz),
                np.gradient(np.gradient(axis, axis=1), axis=1) / math.sqrt(nz),
            )
        )
    ...
    def _decode(cls, augmented: np.ndarray, stage_count: int) -> np.ndarray:
        ...
        return np.asarray(augmented)[:, :size]

    @classmethod
    def representation_roundtrip(
        cls, snapshots: np.ndarray, stage_count: int
    ) -> np.ndarray:
        return cls._decode(cls._augment(np.asarray(snapshots), stage_count), stage_count)
    ...
    def fit(
        ...
        if fixed_rank is None:
            ...
            rank = min(int(fixed_rank), len(singular))
            if rank > cap:
                return None
        return cls(
            mean,
    ...
    def to_dict(self) -> dict[str, Any]:
        value = {
            ...
            "objective_channels": [
                "lossless-primary-physical-residual",
                "cylindrical-energy",
                f"{self.stage_count + 2}-overlapping-smooth-tapered-windows",
                "axis-bz-null-extrema-derivatives",
            ],
