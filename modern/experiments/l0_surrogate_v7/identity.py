"""Detached Git identity and actual-import closure for v7."""
...
SUBJECT = "preregister L0 surrogate v7 experiment"
REMOTE = "origin/feat/sota-foundation"
PREFIXES = (
    "modern/experiments/l0_surrogate_v7/",
    "modern/tests/experiments/l0_surrogate_v7/",
)
...
def runtime_closure(repo: Path) -> list[dict[str, str]]:
    ...
    for name, module in tuple(sys.modules.items()):
        ...
        if kind != "blob" or listed_path != relative:
            raise IdentityError(f"invalid imported module Git object: {name}")
...
def bind(repo: Path, manifest_path: Path) -> Binding:
    ...
    if _git(repo, "symbolic-ref", "-q", "HEAD", check=False):
        raise IdentityError("v7 requires detached HEAD")
    if subprocess.run(
        ...
        raise IdentityError("v7 commit is absent from required remote")
    if _git(repo, "show", "-s", "--format=%s", head) != SUBJECT:
        raise IdentityError("HEAD is not the v7 preregistration commit")
    changed = tuple(
        line
        for line in _git(
    ...
    if not changed or any(not path.startswith(PREFIXES) for path in changed):
        raise IdentityError("v7 preregistration is not exact-path isolated")
    if any("/results/" in path or path.endswith("RESULTS_REPORT.md") for path in changed):
        raise IdentityError("v7 preregistration contains results")
    if _git(repo, "status", "--porcelain=v1", "--untracked-files=all"):
        raise IdentityError("v7 detached worktree is not completely clean")
    closure = runtime_closure(repo)
    external = [
        item
        for item in closure
        if not item["path"].startswith("modern/experiments/l0_surrogate_v7/")
    ]
...
def acquire_lock(repo: Path, commit: str) -> Path:
    ...
    path = common / "l0-surrogate-v7.execution.lock"
    try:
        descriptor = os.open(path, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o444)
    except FileExistsError as error:
        raise IdentityError("exclusive v7 execution lock already exists") from error
