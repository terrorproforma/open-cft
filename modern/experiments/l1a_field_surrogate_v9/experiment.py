"""Lossless physical-residual field surrogate paths for v9."""
...
def raw_designs() -> tuple[Any, ...]:
    ...
    if prior.intersection(tuple(item.values) for item in designs):
        raise RuntimeError("v9 coordinates overlap v1-v8 evidence")
    return designs
...
def construct_geometry(design: Any, raw_index: int) -> tuple[Any, dict[str, Any]]:
    ...
    case = replace(
        case,
        case_id=f"l1a-fs-v9-{raw_index:04d}",
        config_sha256=config_hash,
        case_sha256=case_hash,
    )
    return case, {
        **record,
        "v9_case_id": case.case_id,
        "v9_config_sha256": config_hash,
        "v9_case_sha256": case_hash,
    }
