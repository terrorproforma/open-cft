"""Production geometry, field transfer, GP and weighted-POD paths for v6."""
...
def raw_designs() -> tuple[Any, ...]:
    ...
    if prior.intersection(tuple(item.values) for item in designs):
        raise RuntimeError("v6 coordinates overlap v1/v2/v3/v4/v5 evidence")
    return designs
...
def construct_geometry(design: Any, raw_index: int) -> tuple[Any, dict[str, Any]]:
    ...
    case = replace(
        case,
        case_id=f"l1a-fs-v6-{raw_index:03d}",
        config_sha256=config_hash,
        case_sha256=case_hash,
    )
    return case, {
        **record,
        "v6_case_id": case.case_id,
        "v6_config_sha256": config_hash,
        "v6_case_sha256": case_hash,
    }
