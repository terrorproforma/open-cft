"""Synthetic-preflighted, staged and exactly-once v7 experiment runner."""
...
PREFIXES = (
    "modern/experiments/l1a_field_surrogate_v7/",
    "modern/tests/experiments/l1a_field_surrogate_v7/",
)
DEPENDENCY_ROOTS = (
...
def dependency_lock() -> dict[str, Any]:
    wp.init()
    value = {
        "schema_version": "cft-revival.l1a-field-surrogate-v7.dependency-lock/7.0.0",
        ...
        "v6_development_evidence": {
            "commit": _git("rev-parse", "origin/exp/l1a-field-surrogate-v6"),
            "partitions_blob": _git(
                "rev-parse",
                "origin/exp/l1a-field-surrogate-v6:"
                "modern/experiments/l1a_field_surrogate_v6/partitions.json",
            ),
            "method_freeze_blob": _git(
                "rev-parse",
                "origin/exp/l1a-field-surrogate-v6:"
                "modern/experiments/l1a_field_surrogate_v6/results/"
                "artifacts/frozen-method-selection.json",
            ),
            "terminal_blob": _git(
                "rev-parse",
                "origin/exp/l1a-field-surrogate-v6:"
                "modern/experiments/l1a_field_surrogate_v6/results/terminal.json",
            ),
...
def field_target(data: Mapping[str, np.ndarray], case: Any, family: str) -> np.ndarray:
    ...
    return align_vector(high - low, case) / norm
...
def predict_scalar(
    ...
    prediction = np.empty((len(data_rows), len(QOIS)))
    for column, name in enumerate(QOIS):
        features = np.asarray(
            [
...
def predict_field(
    ...
    low = data["low_field"]
    norm = math.sqrt(max(field_energy(low), 1e-300))
    observed = basis.observed_coefficients(
        (align_vector(low, case) / norm)[None, :]
    )[0]
...
def _synthetic_science_paths() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="l1a-v7-synthetic-science-") as temporary:
        ...
        latency_metrics([0.01] * 48, [0.001] * 48, [0.05] * 48)
        labels = {
            "npz-save-load-qoi": ["save/load/qoi"],
            "snapshot-high-residual": ["observed coarse residual snapshots"],
            "align-unalign": ["piecewise polarity canonicalization"],
            "scalar-high-ratio": ["standardized transformed log ratio"],
    ...
    with tempfile.TemporaryDirectory(prefix="l1a-v7-callback-matrix-") as temporary:
        ...
        for index, expected in enumerate(expected_states):
            ...
            runtime = ExperimentRuntime(
                experiment_id=f"l1a-field-surrogate-v7-synthetic-{expected.value}",
...
def prepare() -> dict[str, Any]:
    ...
    geometry = {
        "schema_version": "cft-revival.l1a-field-surrogate-v7.geometry-preflight/7.0.0",
        "protocol_hash": PROTOCOL_HASH,
    ...
    designs = raw_designs()
    partition = {
        "schema_version": "cft-revival.l1a-field-surrogate-v7.partitions/7.0.0",
        "protocol_hash": PROTOCOL_HASH,
    ...
    synthetic = {
        "schema_version": "cft-revival.l1a-field-surrogate-v7.synthetic-preflight/7.0.0",
        "protocol_hash": PROTOCOL_HASH,
        "geometry_preflight_hash": geometry["geometry_preflight_hash"],
...
def verify_before_runtime() -> VerifiedExecution:
    ...
    attestation = ExecutionAttestation(
        attempt=1,
        commit=head,
        command="python -m experiments.l1a_field_surrogate_v7.run execute",
...
def claim_git_common_attempt(verified: VerifiedExecution) -> dict[str, Any]:
    ...
    payload = {
        "experiment_id": "l1a-field-surrogate-v7",
        "attempt": 1,
    ...
    try:
        with PinnedDirectory.open(namespace) as pinned:
            descriptor = pinned.open_file_exclusive(
                "l1a-field-surrogate-v7.attempt-1.json", 0o600
            )
            try:
...
def persist_phase(
    ...
    value = {
        "schema_version": "cft-revival.l1a-field-surrogate-v7.phase/7.0.0",
        "phase": name,
    ...
    for budget in PROTOCOL["sampling"]["high_budgets"]:
        ...
        for field_family in PROTOCOL["models"]["field_families"]:
            ...
            for stage_count in grouped:
                ...
                basis = WeightedPOD.fit(snapshots, retained_target, rank_cap)
            ...
            for field_length in lengths:
                ...
                if oracle_ok:
                    for stage_count, basis in bases.items():
                        ...
                        observed = np.asarray(
                            [
                                basis.observed_coefficients(
                                    (
                                        align_vector(data["low_field"], case)
                                        / math.sqrt(
        ...
        for (scalar_family, scalar_length), scalar in scalar_results.items():
            for (field_family, field_length), field in field_results.items():
                ...
                candidate = {
                    ...
                    "projection_oracle_passed": field["projection_oracle_passed"],
                    "coefficient_model_fitted": field["coefficient_model_fitted"],
                    "pod_by_stage": field["pod_by_stage"],
                }
                candidate["passed"] = candidate_passes(
                    scalar,
                    regression,
                    field["pod_retained"] >= retained_target,
                    field["projection_oracle_passed"],
                )
...
def execute() -> dict[str, Any]:
    ...
    def prebundle(context: RunContext) -> Mapping[str, Any]:
        ...
        provenance = {
            "schema_version": "cft-revival.l1a-field-surrogate-v7.provenance/7.0.0",
    ...
    def development(context: RunContext) -> Decision:
        ...
        initial_indices = tuple(range(272))
        ...
        checkpoint_record = {
            "schema_version": "cft-revival.l1a-field-surrogate-v7.development-checkpoints/7.0.0",
        ...
        method_freeze = {
            "schema_version": "cft-revival.l1a-field-surrogate-v7.method-freeze/7.0.0",
        ...
        numerical = numerical_gates(state["all_records"], 272)
        if selected is None:
            context.write_json(
    ...
    def assessment(context: RunContext) -> Decision:
        ...
        calibration_record = {
            "schema_version": "cft-revival.l1a-field-surrogate-v7.calibration/7.0.0",
        ...
        numerical = numerical_gates(state["all_records"], 368)
