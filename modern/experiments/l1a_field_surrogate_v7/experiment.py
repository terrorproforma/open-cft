"""Production geometry, field transfer, GP and weighted-POD paths for v7."""
...
def raw_designs() -> tuple[Any, ...]:
    ...
    if prior.intersection(tuple(item.values) for item in designs):
        raise RuntimeError("v7 coordinates overlap v1-v6 evidence")
    return designs
...
def construct_geometry(design: Any, raw_index: int) -> tuple[Any, dict[str, Any]]:
    ...
    case = replace(
        case,
        case_id=f"l1a-fs-v7-{raw_index:03d}",
        config_sha256=config_hash,
        case_sha256=case_hash,
    )
    return case, {
        **record,
        "v7_case_id": case.case_id,
        "v7_config_sha256": config_hash,
        "v7_case_sha256": case_hash,
    }
...
def select_frozen(valid: Mapping[int, Any]) -> tuple[int, ...]:
    ...
    fixed = ordered[:224]
    pool = ordered[224:]
    candidate = [_normalized(valid[index]) for index in fixed]
    ...
    method, remaining = take(pool)
    calibration, remaining = take(remaining)
    assessment, _ = take(remaining)
    frozen = tuple(fixed + method + calibration + assessment)
    if len(frozen) != 368 or len(set(frozen)) != 368:
        raise RuntimeError("unable to freeze 368 unique valid rows")
    return frozen
...
def model_features(
    ...
    if qoi_name == "source_representation_error":
        dr = LOW_DOMAIN.dr_m
        dz = LOW_DOMAIN.dz_m
        phase_values = (
            float(row[1]) / dz,
            float(row[2] * row[1]) / dz,
            float(row[3]) / dr,
            float(row[4]) / dr,
            float(row[5]) / dr,
            float(row[6]) / dr,
...
class SharedKernelGP:
    """Standardized-output ARD Mahalanobis Matérn-5/2 regressor."""
    ...
    def fit(cls, x: np.ndarray, y: np.ndarray, length: float) -> "SharedKernelGP":
        ...
        for column in range(x.shape[1]):
            feature = normalized_x[:, column]
            if float(np.std(feature)) > 1e-12:
                correlations[column] = max(
                    abs(float(np.mean(feature * normalized_y[:, output])))
                    for output in range(normalized_y.shape[1])
                )
        ard_length = float(length) / np.maximum(0.25 + correlations, 0.25)
        kernel = cls.kernel(normalized_x, normalized_x, ard_length)
        kernel.flat[:: len(kernel) + 1] += PROTOCOL["models"]["ridge"]
        alpha = np.linalg.solve(kernel, normalized_y)
        return cls(
            normalized_x,
            alpha,
            x_mean,
...
class WeightedPOD:
    ...
    def fit(
        ...
        total = max(float(np.sum(singular * singular)), 1e-300)
        energy = np.cumsum(singular * singular) / total
    ...
    def observed_coefficients(self, snapshots: np.ndarray) -> np.ndarray:
        return (np.asarray(snapshots) * self.weights) @ self.modes.T
