"""Synthetic-preflighted, staged and exactly-once v5 experiment runner."""
...
PREFIXES = (
    "modern/experiments/l1a_field_surrogate_v5/",
    "modern/tests/experiments/l1a_field_surrogate_v5/",
)
DEPENDENCY_ROOTS = (
...
def dependency_lock() -> dict[str, Any]:
    wp.init()
    value = {
        "schema_version": "cft-revival.l1a-field-surrogate-v5.dependency-lock/5.0.0",
...
def _synthetic_science_paths() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="l1a-v5-synthetic-science-") as temporary:
...
def synthetic_runtime_preflight() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="l1a-v5-callback-matrix-") as temporary:
        matrix_root = Path(temporary)
        for index, expected in enumerate(expected_states):
            root = matrix_root / f"{index}-{expected.value}"
            runtime = ExperimentRuntime(
                experiment_id=f"l1a-field-surrogate-v5-synthetic-{expected.value}",
...
def prepare() -> dict[str, Any]:
    ...
    geometry = {
        "schema_version": "cft-revival.l1a-field-surrogate-v5.geometry-preflight/5.0.0",
        "protocol_hash": PROTOCOL_HASH,
    ...
    designs = raw_designs()
    partition = {
        "schema_version": "cft-revival.l1a-field-surrogate-v5.partitions/5.0.0",
        "protocol_hash": PROTOCOL_HASH,
    ...
    synthetic = {
        "schema_version": "cft-revival.l1a-field-surrogate-v5.synthetic-preflight/5.0.0",
        "protocol_hash": PROTOCOL_HASH,
        "geometry_preflight_hash": geometry["geometry_preflight_hash"],
...
def bind_preregistration() -> tuple[str, list[dict[str, str]]]:
    ...
    if _git("show", "-s", "--format=%s", head) != SUBJECT:
        raise RuntimeError("HEAD is not v5 preregistration")
    if _git("rev-parse", REMOTE) != head:
        raise RuntimeError("remote v5 branch is not the preregistration")
...
def persist_phase(
    ...
    value = {
        "schema_version": "cft-revival.l1a-field-surrogate-v5.phase/5.0.0",
        "phase": name,
...
def execute() -> dict[str, Any]:
    ...
    runtime = ExperimentRuntime(
        experiment_id="l1a-field-surrogate-v5",
        result_root=RESULTS,
        cache_root=CACHE,
        attestation=ExecutionAttestation(
            attempt=1,
            commit=commit,
            command="python -m experiments.l1a_field_surrogate_v5.run execute",
            device=PROTOCOL["execution"]["device"],
            clean_worktree=not bool(
    ...
    def prebundle(context: RunContext) -> Mapping[str, Any]:
        preregistration, closure = bind_preregistration()
        ...
        provenance = {
            "schema_version": "cft-revival.l1a-field-surrogate-v5.provenance/5.0.0",
            "preregistration_commit": preregistration,
            "accepted_experiment_runtime_commit": ACCEPTED_RUNTIME_COMMIT,
            "protocol_hash": PROTOCOL_HASH,
            "dependency_lock_hash": strict_load(DEPENDENCY_LOCK)[
    ...
    def development(context: RunContext) -> Decision:
        ...
        method_freeze = {
            "schema_version": "cft-revival.l1a-field-surrogate-v5.method-freeze/5.0.0",
    ...
    def assessment(context: RunContext) -> Decision:
        ...
        calibration_record = {
            "schema_version": "cft-revival.l1a-field-surrogate-v5.calibration/5.0.0",
