...
# Raw-detection labels of the characterization v1 protocol that mark an axis root member.
V1_AXIS_DETECTION_METHODS = ("axis_sign_change", "axis_grid")
...
def v1_axis_reference(recorded: Mapping[str, Any]) -> list[dict[str, Any]]:
    """Sealed v1 primary-map axis roots: clusters that contain an axis-detected member.

    v3.1 correction (v3 POSTHOC_AUDIT): v1 merges an ``axis_sign_change`` detection with a
    bilinear Newton root of the neighbouring cell when both lie within 0.75 mesh cells and
    reports the centroid, so the cluster centroid ``r_m`` is not exactly 0.0 for 26 of the 206
    sealed axis clusters. The v3 filter ``r_m == 0.0`` dropped them and failed its held-out
    gate for 14 designs; the reference is defined by the members' detection method instead.
    """
    ...
    return [
        {
            ...
            "z_m": root["z_m"],
            "centroid_r_m": root["r_m"],
            "methods": list(root["methods"]),
            "member_count": int(root["member_count"]),
        ...
        if not root["finite_box_boundary"]
        and any(member["method"] in V1_AXIS_DETECTION_METHODS for member in root["members"])
    ]
...
def _resolve_characterization(spec: DesignSpec, protocol: Mapping[str, Any]) -> ResolvedDesign:
    ...
    primary_roots = v1_axis_reference(recorded)
    evidence = {
        "identity_checks": checks,
        "accepted_solve": _solver_evidence(accepted_field),
        "refined_solve": _solver_evidence(refined_field),
