"""Synthetic-preflighted, staged and exactly-once v8 experiment runner."""
...
PREFIXES = (
    "modern/experiments/l1a_field_surrogate_v8/",
    "modern/tests/experiments/l1a_field_surrogate_v8/",
)
DEPENDENCY_ROOTS = (
...
def dependency_lock() -> dict[str, Any]:
    wp.init()
    value = {
        "schema_version": "cft-revival.l1a-field-surrogate-v8.dependency-lock/8.0.0",
        ...
        "v7_development_evidence": {
            "commit": _git("rev-parse", "origin/exp/l1a-field-surrogate-v7"),
            "partitions_blob": _git(
                "rev-parse",
                "origin/exp/l1a-field-surrogate-v7:"
                "modern/experiments/l1a_field_surrogate_v7/partitions.json",
            ),
            "method_freeze_blob": _git(
                "rev-parse",
                "origin/exp/l1a-field-surrogate-v7:"
                "modern/experiments/l1a_field_surrogate_v7/results/"
                "artifacts/frozen-method-selection.json",
            ),
            "terminal_blob": _git(
                "rev-parse",
                "origin/exp/l1a-field-surrogate-v7:"
                "modern/experiments/l1a_field_surrogate_v7/results/terminal.json",
            ),
...
def predict_field(
    ...
    observed = encoder.whiten(encoder.project(
    ...
    with tempfile.TemporaryDirectory(prefix="l1a-v7-callback-matrix-") as temporary:
        ...
        for index, expected in enumerate(expected_states):
            ...
            runtime = ExperimentRuntime(
                experiment_id=f"l1a-field-surrogate-v8-synthetic-{expected.value}",
...
def prepare() -> dict[str, Any]:
    ...
    geometry = {
        "schema_version": "cft-revival.l1a-field-surrogate-v8.geometry-preflight/8.0.0",
        "protocol_hash": PROTOCOL_HASH,
    ...
    partition = {
        "schema_version": "cft-revival.l1a-field-surrogate-v8.partitions/8.0.0",
        "protocol_hash": PROTOCOL_HASH,
    ...
    synthetic = {
        "schema_version": "cft-revival.l1a-field-surrogate-v8.synthetic-preflight/8.0.0",
        "protocol_hash": PROTOCOL_HASH,
        "geometry_preflight_hash": geometry["geometry_preflight_hash"],
...
def verify_before_runtime() -> VerifiedExecution:
    ...
    attestation = ExecutionAttestation(
        attempt=1,
        commit=head,
        command="python -m experiments.l1a_field_surrogate_v8.run execute",
...
def claim_git_common_attempt(verified: VerifiedExecution) -> dict[str, Any]:
    ...
    payload = {
        "experiment_id": "l1a-field-surrogate-v8",
        "attempt": 1,
    ...
    try:
        with PinnedDirectory.open(namespace) as pinned:
            descriptor = pinned.open_file_exclusive(
                "l1a-field-surrogate-v8.attempt-1.json", 0o600
            )
            try:
...
def persist_phase(
    ...
    value = {
        "schema_version": "cft-revival.l1a-field-surrogate-v8.phase/8.0.0",
        "phase": name,
...
def fit_development_candidates(
    ...
    for budget in PROTOCOL["sampling"]["high_budgets"]:
        ...
        for data, case in zip(training, training_cases, strict=True):
            truth = data["high_field"]
            restored = unalign_vector(align_vector(truth, case), case)
            alignment_rows.append(
                float(
                    np.linalg.norm(restored - truth)
                    / max(np.linalg.norm(truth), 1e-300)
                )
            )
        alignment_max = max(alignment_rows)
        ...
        for stage_count in grouped:
            ...
            basis = WeightedPOD.fit(
                snapshots, retained_target, rank_cap, stage_count
            )
        ...
        oracle_ok = bool(
            oracle_metric_ok
            and alignment_max <= PROTOCOL["models"]["alignment_roundtrip_l2_max"]
        )
        ...
        candidate = {
            ...
            "alignment_roundtrip_max_l2": alignment_max,
            "alignment_roundtrip_passed": alignment_max
            <= PROTOCOL["models"]["alignment_roundtrip_l2_max"],
        ...
        candidate["passed"] = bool(
            candidate["alignment_roundtrip_passed"]
...
def execute() -> dict[str, Any]:
    ...
    def prebundle(context: RunContext) -> Mapping[str, Any]:
        ...
        provenance = {
            "schema_version": "cft-revival.l1a-field-surrogate-v8.provenance/8.0.0",
    ...
    def development(context: RunContext) -> Decision:
        ...
        checkpoint_record = {
            "schema_version": "cft-revival.l1a-field-surrogate-v8.development-checkpoints/8.0.0",
        ...
        method_freeze = {
            "schema_version": "cft-revival.l1a-field-surrogate-v8.method-freeze/8.0.0",
    ...
    def assessment(context: RunContext) -> Decision:
        ...
        calibration_record = {
            "schema_version": "cft-revival.l1a-field-surrogate-v8.calibration/8.0.0",
