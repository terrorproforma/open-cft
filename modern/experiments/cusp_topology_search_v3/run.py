...
Lifecycle (from ``modern/``)::
    ...
    python -m experiments.cusp_topology_search_v3.run shakedown  # any HEAD, BEFORE prepare
    python -m experiments.cusp_topology_search_v3.run prepare    # refuses without a valid shakedown.json
...
SUBJECT = "preregister cusp topology search v3"
REMOTE_BRANCH = "origin/exp/cusp-topology-search-v3"
COMMAND = "python -m experiments.cusp_topology_search_v3.run execute"
SHAKEDOWN_COMMAND = "python -m experiments.cusp_topology_search_v3.run shakedown"
...
EXPERIMENT_PREFIX = "modern/experiments/cusp_topology_search_v3/"
TESTS_PREFIX = "modern/tests/experiments/cusp_topology_search_v3/"
...
def shakedown() -> dict[str, Any]:
    ...
    temp_root = Path(tempfile.gettempdir()) / f"cts-v3-shakedown-{git['head'][:12]}-{stamp}-{token}"
...
def execute() -> dict[str, Any]:
    ...
    value = protocol()
    cache_root = Path(tempfile.gettempdir()) / f"cts-v3-{commit[:12]}-working-cache"
    runtime = ExperimentRuntime(
