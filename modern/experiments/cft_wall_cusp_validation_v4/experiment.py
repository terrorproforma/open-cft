...
SCHEMA_VERSION = "cft-revival.cft-wall-cusp-validation-v4.dataset/1.0.0"
...
def run_serialization_preflight() -> dict[str, Any]:
    ...
    return {
        "schema_version": "cft-revival.cft-wall-cusp-validation-v4.serialization-preflight/1.0.0",
        "status": "passed",
...
def dependency_closure() -> dict[str, Any]:
    ...
    paths = tuple(
        ...
        if (
            ...
            or path.startswith("modern/experiments/cft_wall_cusp_validation_v4/")
        )
...
def case_definitions() -> tuple[CaseDefinition, ...]:
    ...
    for stages, pitch_index, radius_index, polarity in itertools.product(
        ...
        case_id = (
            f"wcval-v4-s{int(stages):02d}-p{pitch_index}-r{radius_index}-{sign}"
        )
...
class HeldOutArtifactAdapter:
    adapter_id = "experiments.cft-wall-cusp-validation-v4.held-out-artifact"
...
def _report(dataset: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Coupling v4.2 wall-cusp held-out validation v4",
        "",
...
class ValidationCallbacks:
    ...
    def development(self, context: RunContext) -> Decision:
        freeze = {
            "schema_version": "cft-wall-cusp-v4.threshold-freeze/1.0.0",
