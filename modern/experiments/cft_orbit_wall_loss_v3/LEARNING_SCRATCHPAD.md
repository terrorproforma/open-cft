...
- Task: preregister and execute nine-authority CFT full-orbit wall-loss v3.
...
- [tool] Orbit launch seeds are unsigned 64-bit values while shared runtime JSON
  accepts signed 64-bit integers. Each exact v3 payload builder encodes seeds
...
- [user] V1 accessed zero P2 fields/outcomes; v2 accessed three P2 maps and
  constructed 32 in-memory orbit results before its campaign-prefix failure.
- [self] Checkpoint launch IDs must be created with
  `ensemble_id == campaign_id`; never rename post-construction because seeds
  bind the exact full launch ID.
- [user] V3 uses eight fresh position samples and a pi/16 gyrophase rotation.
  The preflight must prove zero v2 identity, seed, position and phase-space
  overlap and validate partial/resumed/final chains for every case.
