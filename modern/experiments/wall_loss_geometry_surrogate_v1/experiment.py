"""Preregistered wall-loss geometry surrogate v1: contracts, plans, callbacks and gates.

Classification ``SURROGATE_OF_SCREENING_L1A_FIELD_TEST_PARTICLE_WALL_LOSS``: a
learned surrogate of the SCREENING dataset of
``orbit_wall_loss_geometry_screening_v1``; never physical-orbit evidence, never a
performance model.  One :class:`CampaignPlan` drives the evidentiary campaign and
the disclosed NON-EVIDENTIARY shakedown (different partition seed namespace) so
the shakedown exercises exactly the production code.
"""
...
from .predictor import CLASSIFICATION, PREDICTOR_SCHEMA, SOURCE_CLASSIFICATION, Predictor
...
VERSION_TAG = "cft-revival.wall-loss-geometry-surrogate-v1"
PACKAGES = ("torch", "botorch", "gpytorch", "numpy", "scipy")
GATED_OUTPUTS = ("p_wall_cell1", "p_wall_cell2", "p_wall_cell3", "p_wall_cell4", "p_wall_pooled")
ALL_OUTPUTS = GATED_OUTPUTS + ("p_reflect_pooled",)
STRUCTURAL_GATES = (
    "no_tautology",
    "determinism_replay",
    "dataset_binding",
    "partition_frozen_single_use",
    "predictor_contract_replay",
    "code_contract",
)
SCIENCE_GATES = ("interpolation_rmse_pooled", "interpolation_rmse_cells", "beats_best_baseline_2x", "coverage_90")
...
def _plain(value: Any) -> Any:
    """Plain JSON types only (numpy scalars/arrays and tuples are converted)."""
    ...
    if isinstance(value, float) and not math.isfinite(value):
        raise ValueError("non-finite value in a published payload")
    return value
...
def protocol_consistency(value: Mapping[str, Any]) -> dict[str, bool]:
    ...
    checks["candidate_transforms"] = all(
        (("logit" in name) == (m.TRANSFORM_OF[name] == "logit")) for name in m.CANDIDATE_ORDER
    )
    checks["outputs"] = (
    ...
    checks["inputs"] = len(value["inputs"]["names"]) == 11 == len(value["inputs"]["units"]) and list(value["inputs"]["names"]) == sorted(value["inputs"]["names"])
    ...
    checks["partition_counts"] = (
        ...
        and all(
            int(counts["totals"][role]) == sum(int(counts[stratum][role]) for stratum in ("primary", "extension"))
            for role in d.ROLES
        )
    ...
    checks["code_contract_scope"] = all(
        f"modern/experiments/wall_loss_geometry_surrogate_v1/{name}" in scope
        for name in ("__init__.py", "data.py", "models.py", "predictor.py", "experiment.py", "run.py")
    ) and "modern/src/cft_revival/surrogates/*.py" in scope and "modern/src/cft_revival/experiment_runtime/*.py" in scope
    checks["torch_policy"] = (
        value["candidates"]["determinism"]["torch"]["device"] == "cpu"
        and int(value["candidates"]["determinism"]["torch"]["threads"]) <= 8
    )
    checks["active_learning"] = value["active_learning_addon"]["model"] == "pkg-exactgp-logit" and value["active_learning_addon"]["pool_role"] == "fit"
    return checks
...
# --------------------------------------------------------------------------
# Plans and the frozen partition
# --------------------------------------------------------------------------
...
class CampaignPlan:
    ...
    def __post_init__(self) -> None:
        ...
        if (self.kind == "evidentiary") != self.binding_gates:
            raise ValueError("only the evidentiary plan binds gates")
...
def load_rows(value: Mapping[str, Any]) -> tuple[d.DesignRow, ...]:
    return d.load_rows(value["dataset"], value["inputs"]["names"], value["outputs"])
...
def shakedown_disjointness(value: Mapping[str, Any], rows: Sequence[d.DesignRow]) -> dict[str, Any]:
    ...
    proven = bool(namespace_differs and not overlap["identical"] and evidentiary["role_sha256"] != shakedown["role_sha256"])
    return {
        "proven": proven,
        ...
        "note": "the extrapolation cluster is a deterministic function of the data (top-decile chamber length) and is therefore identical in both plans; the four interpolation roles are reshuffled",
...
def verify_shakedown_record(value: Mapping[str, Any], record: Mapping[str, Any]) -> dict[str, bool]:
    ...
    try:
        ...
        checks["evidentiary_design_sha256_current"] = (
            record.get("evidentiary_design_sha256") == recomputed["evidentiary_design_sha256"]
        )
...
def _z(probability: float) -> float:
    return NormalDist().inv_cdf(0.5 + probability / 2.0)


def score_candidate_on_rows(
    candidate: m.FittedCandidate, table: m.TrainingTable, rows: Sequence[d.DesignRow], outputs: Sequence[str]
) -> dict[str, Any]:
    """Raw P-unit RMSE per output (no intervals; used for method selection)."""

    physical = m.physical_matrix(rows)
    normalized = table.normaliser.transform(physical)
    per_output = {}
    for name in outputs:
        truth = np.asarray([row.counts[name][0] / row.counts[name][1] for row in rows])
        predicted = m.candidate_probabilities(candidate, normalized, name)
        per_output[name] = m.rmse(predicted - truth)
    per_output["mean_over_outputs"] = float(np.mean([per_output[name] for name in outputs]))
    return per_output


def score_baseline_on_rows(
    baseline: m.Baseline, table: m.TrainingTable, rows: Sequence[d.DesignRow], outputs: Sequence[str]
) -> dict[str, Any]:
    normalized = table.normaliser.transform(m.physical_matrix(rows))
    per_output = {}
    cell_errors: list[float] = []
    for name in outputs:
        truth = np.asarray([row.counts[name][0] / row.counts[name][1] for row in rows])
        errors = baseline.predict(normalized, name) - truth
        per_output[name] = m.rmse(errors)
        if name in m.CELL_OUTPUTS:
            cell_errors.extend(errors.tolist())
    per_output["mean_over_outputs"] = float(np.mean([per_output[name] for name in outputs]))
    per_output["cells_pooled"] = m.rmse(cell_errors)
    return per_output


def scope_assessment(
    predictor: Predictor,
    rows: Sequence[d.DesignRow],
    outputs: Sequence[str],
    gated: Sequence[str],
    baselines: Mapping[str, m.Baseline],
    table: m.TrainingTable,
    scope_name: str,
) -> dict[str, Any]:
    """Predictions, truths, intervals and metrics of one scope (single label read by the caller)."""

    physical = m.physical_matrix(rows)
    predictions = predictor.predict(physical.tolist())
    normalized = table.normaliser.transform(physical)
    per_design = []
    errors: dict[str, list[float]] = {name: [] for name in outputs}
    covered: dict[str, int] = {name: 0 for name in outputs}
    latent_covered: dict[str, int] = {name: 0 for name in outputs}
    labels: dict[str, list[tuple[int, int]]] = {name: [] for name in outputs}
    for row, prediction in zip(rows, predictions, strict=True):
        record = {"case_id": row.case_id, "batch": row.batch, "chamber_length_m": row.chamber_length_m, "stage_count": row.stage_count, "scope": prediction["scope"], "outputs": {}}
        for name in outputs:
            successes, trials = row.counts[name]
            truth = successes / trials
            item = prediction["outputs"][name]
            low, high = item["observation_interval"]
            l_low, l_high = item["latent_interval"]
            error = item["probability"] - truth
            errors[name].append(error)
            labels[name].append((successes, trials))
            covered[name] += int(low <= truth <= high)
            latent_covered[name] += int(l_low <= truth <= l_high)
            record["outputs"][name] = {
                "truth": truth,
                "successes": successes,
                "trials": trials,
                "predicted": item["probability"],
                "error": error,
                "observation_interval": [low, high],
                "latent_interval": [l_low, l_high],
                "covered": low <= truth <= high,
                "working_mean": item["working_mean"],
                "working_total_variance_calibrated": item["working_total_variance_calibrated"],
            }
        per_design.append(record)
    per_output = {}
    for name in outputs:
        floor = d.binomial_floor(labels[name])
        raw = m.rmse(errors[name])
        per_output[name] = {
            "rmse": raw,
            "binomial_floor": floor,
            "rmse_floor_corrected": d.floor_corrected(raw, floor),
            "mae": float(np.mean(np.abs(errors[name]))),
            "worst_abs_error": float(np.max(np.abs(errors[name]))),
            "coverage_observation_interval": covered[name] / len(rows),
            "coverage_latent_interval": latent_covered[name] / len(rows),
            "count": len(rows),
        }
    cell_errors = [e for name in m.CELL_OUTPUTS for e in errors[name]]
    cell_labels = [lab for name in m.CELL_OUTPUTS for lab in labels[name]]
    cell_floor = d.binomial_floor(cell_labels)
    cell_raw = m.rmse(cell_errors)
    gated_count = sum(len(errors[name]) for name in gated)
    gated_covered = sum(covered[name] for name in gated)
    baseline_scores = {
        baseline_id: score_baseline_on_rows(baseline, table, rows, outputs) for baseline_id, baseline in baselines.items()
    }
    best_pooled = min(baseline_scores, key=lambda b: (baseline_scores[b]["p_wall_pooled"], b))
    best_cells = min(baseline_scores, key=lambda b: (baseline_scores[b]["cells_pooled"], b))
    return {
        "scope": scope_name,
        ...
        "cells": {
            "rmse": cell_raw,
            "binomial_floor": cell_floor,
            "rmse_floor_corrected": d.floor_corrected(cell_raw, cell_floor),
            "count": len(cell_errors),
            "coverage_observation_interval": sum(covered[name] for name in m.CELL_OUTPUTS) / max(1, len(cell_errors)),
        },
        "gated_coverage": {
            "covered": gated_covered,
            "count": gated_count,
            "coverage": gated_covered / max(1, gated_count),
            "nominal_probability": predictor.nominal_probability,
        },
        "baselines": baseline_scores,
        "best_baseline_pooled": {
            "baseline": best_pooled,
            "rmse": baseline_scores[best_pooled]["p_wall_pooled"],
            "ratio_to_surrogate": (
                baseline_scores[best_pooled]["p_wall_pooled"] / per_output["p_wall_pooled"]["rmse"]
                if per_output["p_wall_pooled"]["rmse"] > 0.0
                else math.inf
            ),
        },
        "best_baseline_cells": {
            "baseline": best_cells,
            "rmse": baseline_scores[best_cells]["cells_pooled"],
            "ratio_to_surrogate": (
                baseline_scores[best_cells]["cells_pooled"] / cell_raw if cell_raw > 0.0 else math.inf
            ),
        },
        "designs": per_design,
    }
...
def no_tautology_report(rows: Sequence[d.DesignRow], input_names: Sequence[str], fit_rows: Sequence[d.DesignRow]) -> dict[str, Any]:
    """(a) counts, (b) no target is affine in a single input, (c) ridge on inputs stays above the floor."""
    ...
    for name in GATED_OUTPUTS:
        ...
        for column in range(x_all.shape[1]):
            ...
            affine_max_r2 = max(affine_max_r2, r2)
    table = m.TrainingTable.build(fit_rows, input_names, GATED_OUTPUTS, {name: fit_rows[0].counts[name][1] for name in GATED_OUTPUTS})
    ridge = m.fit_ridge(table, 1e-6)
    ridge_rmse = {}
    for name in GATED_OUTPUTS:
        truth = table.probabilities(name)
        ridge_rmse[name] = m.rmse(ridge.predict(table.normalized, name) - truth)
    ...
    return {
        ...
        "ridge_in_sample_rmse_fit_role": ridge_rmse,
        "binomial_floor_fit_role": floors,
        "ridge_stays_above_binomial_floor": ridge_above_floor,
        "statement": "targets are orbit-termination counts from orbit_mc integration in L1a re-solved fields; no algebraic function of the inputs produces them",
...
def build_callbacks(
    ...
    *,
    ...
    def prebundle(context: Any) -> Mapping[str, Any]:
        ...
        rows = load_rows(value)
        degeneracy = d.degeneracy_report(rows, input_names)
        if not degeneracy["passed"]:
            raise ValueError("inputs are degenerate")
        partition = plan_partition(value, plan, rows)
        disjointness = shakedown_disjointness(value, rows)
        if not disjointness["proven"]:
            raise ValueError("shakedown/evidentiary partitions are not disjoint")
        ...
        context.write_json("artifacts/dataset-binding.json", _plain({**binding, "degeneracy": degeneracy}))
        ...
        state.update({"rows": rows, "partition": partition, "contract": contract, "binding": binding})
    ...
    def development(context: Any) -> Decision:
        ...
        # ---- label access (fit / method-selection / calibration) --------------
        role_rows: dict[str, tuple[d.DesignRow, ...]] = {}
        for role in ("fit", "method-selection", "calibration"):
            context.before_expensive(
                f"labels-{role}",
                kind="label",
                details={"role": role, "count": len(partition["roles"][role]), "role_sha256": partition["role_sha256"][role]},
            )
            ...
            _plain(
                {
                    ...
                    "method_selection_case_ids": [row.case_id for row in role_rows["method-selection"]],
        ...
        for candidate_id in m.CANDIDATE_ORDER:
            ...
            selection_score = score_candidate_on_rows(fitted, table, role_rows["method-selection"], GATED_OUTPUTS)
            fit_score = score_candidate_on_rows(fitted, table, fit_rows, GATED_OUTPUTS)
            candidate_report[candidate_id] = _plain(
                {
                    "transform": fitted.transform,
                    "diagnostics": fitted.diagnostics,
                    "method_selection_rmse": selection_score,
                    "fit_role_in_sample_rmse": fit_score,
        ...
        # ---- baselines (ridge penalty chosen on the method-selection role) -----
        ridge_scores = {}
        for penalty in m.RIDGE_PENALTIES:
            ridge = m.fit_ridge(table, penalty)
            ridge_scores[penalty] = score_baseline_on_rows(ridge, table, role_rows["method-selection"], GATED_OUTPUTS)["mean_over_outputs"]
        ...
        baseline_report = {
            baseline_id: {
                "parameters": _plain({k: v for k, v in baseline.parameters.items() if k != "weights"}),
                "method_selection_rmse": score_baseline_on_rows(baseline, table, role_rows["method-selection"], GATED_OUTPUTS),
            }
        ...
        contract = _plain(
            {
                ...
                "dataset_binding": {
                    ...
                    for key in ("dataset_file_sha256", "manifest_file_sha256", "screening_result_commit", "screening_preregistration_commit", "screening_merge_commit")
                },
                "inputs": {
                    "names": list(input_names),
                    "units": list(value["inputs"]["units"]),
                    "normaliser": table.normaliser.to_dict(),
                    "known_discontinuities": value["inputs"]["known_discontinuities"],
                },
                "interpolation_scope": {
                    "statement": "inputs inside the fit-role unit box and realised chamber length below the extrapolation threshold (top-decile chamber lengths were held out); predictions outside carry the extrapolation caveat",
                ...
                "calibration": {
                    ...
                    "nominal_probability": 0.9,
                    "fit_sample_count": calibrator.fit_sample_count,
                    "applies_to": "total predictive variance (latent + prediction-time binomial noise)",
                },
                "observation_noise_rule": value["outputs"]["known_noise"]["prediction_time_observation_noise"],
                "prediction_rule": "probability = inverse transform of the working latent mean; latent interval = inverse transform of mean +/- z_0.90 sqrt(scale * latent variance); observation interval = inverse transform of mean +/- z_0.90 sqrt(scale * (latent variance + binomial noise at the predicted mean))",
            }
        )
        ...
        # ---- predictor replay against the native posterior (fit-role + calibration inputs) ----
        ...
        sensitivity = {
            ...
            "permutation_importance": m.permutation_importance(
                selected, table, GATED_OUTPUTS, repeats=int(value["sensitivity"]["permutation_importance"]["repeats"]), namespace="wall-loss-geometry-surrogate-v1:permutation"
            ),
            "interpretation": "sensitivities of the SCREENING dataset's wall-loss probability to the geometry-v1.1 design variables; not physical sensitivities",
        }
        # ---- active learning add-on ----------------------------------------
        al_spec = value["active_learning_addon"]
        active_learning: dict[str, Any]
        if al_spec.get("enabled"):
            context.before_expensive("active-learning-addon", kind="solver", details={"pool": len(fit_rows), "budgets": list(al_spec["budgets"])})
            _log("development: active-learning add-on")
            active_learning = m.active_learning_study(fit_rows, role_rows["method-selection"], input_names, GATED_OUTPUTS, trials, al_spec)
        else:
        ...
        context.write_json("artifacts/active-learning.json", _plain(active_learning))
        ...
        accepted = bool(
            environment["float64_cholesky_probe"]
            and len(candidates) == len(m.CANDIDATE_ORDER)
            and predictor_replay["passed"]
            and determinism["passed"]
            and tautology["passed"]
        )
        collector["development"] = {
            ...
            "active_learning": active_learning.get("comparison"),
        }
        return Decision(
            accepted,
            {
    ...
    def assessment(context: Any) -> Decision:
        ...
        # ---- single-use label reads ------------------------------------------
        scopes = {}
        for role, scope_name in (("assessment", "interpolation"), ("extrapolation", "extrapolation")):
            context.before_expensive(
                f"labels-{role}",
                kind="label",
                details={"role": role, "count": len(partition["roles"][role]), "role_sha256": partition["role_sha256"][role], "single_use": True},
            )
            scope_rows = d.labels_for_role(rows, partition, role)
            scopes[scope_name] = scope_assessment(predictor, scope_rows, ALL_OUTPUTS, GATED_OUTPUTS, baselines, table, scope_name)
        ...
        assessment_physical = np.vstack(
            [m.physical_matrix(d.labels_for_role(rows, partition, "assessment")), m.physical_matrix(d.labels_for_role(rows, partition, "extrapolation"))]
        )
        ...
        # ---- label access order (from the runtime's own access records) -----
        access = _access_records(context)
        label_sequence = [item["details"].get("role") for item in access if item["kind"] == "label"]
        predictor_written_before_assessment = True  # the predictor artifact is written in development, which precedes this stage by construction
        single_use = (
        ...
        binding = {
            ...
            "dataset_binding": {"passed": bool(state["binding"]["passed"]), "checks": state["binding"]["checks"]},
            "partition_frozen_single_use": {
                "passed": bool(single_use and (frozen is None or semantic_sha256(frozen.partitions) == semantic_sha256(partition))),
                "label_access_sequence": label_sequence,
                "single_use": single_use,
                "predictor_written_before_assessment": predictor_written_before_assessment,
                "frozen_partition_matches": frozen is None or semantic_sha256(frozen.partitions) == semantic_sha256(partition),
            },
        ...
        # The evidentiary plan is decided by every binding gate; the shakedown plan is
        # decided by the structural gates only (its science gates are informational).
...
def _access_records(context: Any) -> list[dict[str, Any]]:
    """Read the runtime's committed access records (sequence order)."""

    records = []
    access_root = Path(context.store.root) / "access"
    ...
    for path in sorted(access_root.glob("*.json")):
        ...
        records.append({"sequence": row["sequence"], "operation": row["operation"], "kind": row["kind"], "details": row["details"]})
