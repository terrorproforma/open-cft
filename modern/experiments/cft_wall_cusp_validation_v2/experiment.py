...
SCHEMA_VERSION = "cft-revival.cft-wall-cusp-validation-v2.dataset/1.0.0"
MANIFEST_VERSION = "cft-revival.cft-wall-cusp-validation-v2.manifest/1.0.0"
...
def _json_value(value: Any) -> Any:
    ...
    if isinstance(value, float) and not math.isfinite(value):
        raise ValueError("canonical result serialization rejects nonfinite floats")
    return value
...
def run_serialization_preflight() -> dict[str, Any]:
    """Exercise canonical serialization without accessing v1/v2 held-out maps."""
    ...
    if canonical_bytes(decoded) != encoded:
        raise ValueError("serialization preflight is not canonical")
    return {
        "schema_version": "cft-revival.cft-wall-cusp-validation-v2.serialization-preflight/1.0.0",
        "status": "passed",
        "held_out_v1_or_v2_map_access_count": 0,
        "matrix_case_ids": tuple(matrix),
        "serialized_dataclass_types": _dataclass_type_names(matrix),
...
def case_definitions() -> tuple[CaseDefinition, ...]:
    ...
    for stages, pitch_index, radius_index, polarity in itertools.product(
        ...
        case_id = (
            f"wcval-v2-s{int(stages):02d}-p{pitch_index}-r{radius_index}-{sign}"
        )
...
class AcceptedL1aAdapter:
    adapter_id = "experiments.cft-wall-cusp-validation-v2.accepted-l1a-v4"
...
class MapGuidingCenterOrbitAdapter:
    ...
    def __init__(self, map_set: Any, registrations: tuple[CFTCellRegistration, ...]) -> None:
        ...
        self.orbit_code_hash = hashlib.sha256(
            b"cft-wall-cusp-map-orbit-model-v2\0" + bytes.fromhex(source_hash)
        ).hexdigest()
...
class HeldOutArtifactAdapter:
    adapter_id = "experiments.cft-wall-cusp-validation-v2.held-out-artifact"
...
def _report(dataset: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Coupling v4 wall-cusp held-out validation v2",
        "",
...
def _checkpoint(
    ...
    write_semantic_json(
        path,
        {
            "schema_version": "cft-revival.cft-wall-cusp-validation-v2.phases/1.0.0",
            "attempt": 1,
            "events": events,
        },
    )
...
def _access(
    ...
    row = {
        "sequence": sum(
            1
            for _ in (output / "access-log.jsonl").open(
                "r", encoding="utf-8"
            )
        )
        + 1
        if (output / "access-log.jsonl").exists()
        else 1,
    ...
    with (output / "access-log.jsonl").open(
...
def run_experiment(output_dir: Path) -> dict[str, Any]:
    ...
    lock_payload = {
        ...
        "command": os.environ.get(
            "CFT_ATTEMPT_COMMAND",
            "python -m experiments.cft_wall_cusp_validation_v2.worker",
        ),
        "host": platform.node(),
        "process_id": os.getpid(),
        "acquired_at_utc": datetime.now(timezone.utc),
        "declared_device": PROTOCOL["maps"]["solver"]["device"],
    ...
    write_semantic_json(output / "dependency-closure.json", closure)
    _checkpoint(
        output,
        "initialization",
        "complete",
    ...
    _checkpoint(output, "runtime_identity", "complete", runtime)
...
def finalize_attempt(
    ...
    *,
    ...
    """Finalize either success or typed failure after the sole worker process."""

    output = output_dir.resolve()
    if not output.exists():
        raise RuntimeError("worker did not acquire the results lock")
    (output / ".gitattributes").write_text(
        "* -text\n", encoding="utf-8", newline="\n"
    )
    (output / "stdout.log").write_bytes(stdout_bytes)
    (output / "stderr.log").write_bytes(stderr_bytes)
    lock = _strict_json(output / "execution-lock.json")
    lock.pop("lock_semantic_sha256", None)
    lock.update(
        {
            "command": command,
            "exit_code": exit_code,
            ...
            "status": (
                ...
                if exit_code == 0
                else "single_execution_failed_immutable"
            ),
        }
    )
    lock["lock_semantic_sha256"] = semantic_hash(lock)
    (output / "execution-lock.json").write_text(
        json.dumps(_json_value(lock), indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
        newline="\n",
    )
    attempt = write_semantic_json(
        output / "attempt.json",
        {
            "schema_version": "cft-revival.cft-wall-cusp-validation-v2.attempt/1.0.0",
            "attempt": 1,
            "command": command,
            "exit_code": exit_code,
            "stdout_sha256": hashlib.sha256(stdout_bytes).hexdigest(),
            ...
            "host": lock["host"],
            "declared_device": lock["declared_device"],
            "preregistration_commit_sha": lock["preregistration_commit_sha"],
            "clean_worktree_attested": lock["clean_worktree_attested"],
            "detached_head_attested": lock["detached_head_attested"],
        },
    )
    if exit_code != 0:
        phase = (
            load_semantic_json(output / "phase-status.json")
        ...
        access_rows = (
            [
                json.loads(line)
                for line in (output / "access-log.jsonl").read_text(
                    encoding="utf-8"
                ).splitlines()
                if line
            ]
        ...
        failure_summary = {
            ...
            "materialized_map_count": len(list((output / "fields").rglob("*-field.json")))
            if (output / "fields").exists()
            else 0,
            "prerecord_count": len(prerecords),
            "gpu_replay_count": len(list((output / "gpu-replay").glob("*.json")))
            if (output / "gpu-replay").exists()
            else 0,
            "candidate_cell_count": sum(
                int(item["candidate_cell_count"]) for item in summaries
            ),
            "resolved_cell_count": sum(
                int(item["resolved_cell_count"]) for item in summaries
            ),
            "candidate_path_count": sum(
                int(item["candidate_path_count"]) for item in summaries
            ),
            "resolved_path_count": sum(
                int(item["resolved_path_count"]) for item in summaries
            ),
            "candidate_orbit_count": sum(
                int(item["candidate_orbit_count"]) for item in summaries
            ),
            "resolved_orbit_count": sum(
                int(item["resolved_orbit_count"]) for item in summaries
            ),
        ...
        failure = write_semantic_json(
            output / "failure.json",
            {
                "schema_version": "cft-revival.cft-wall-cusp-validation-v2.failure/1.0.0",
                "status": "failed_immutable_no_patch_no_rerun",
                "attempt": attempt,
                "last_phase": events[-1] if events else None,
                "phase_event_count": len(events),
                "access_log_sha256": hashlib.sha256(
                    (output / "access-log.jsonl").read_bytes()
                ).hexdigest()
                if (output / "access-log.jsonl").exists()
                else None,
                "stderr_tail": stderr_bytes.decode("utf-8", errors="replace")[-8000:],
                "summary": failure_summary,
                "v1_disclosure": PROTOCOL["held_out_family"][
                    "excluded_accessed_evidence"
                ]["disclosure"],
            },
        )
        (output / "report.md").write_text(
            "\n".join(
                (
                    ...
                    "",
                    "The sole clean-detached attempt failed and was not patched or rerun.",
                    "",
                    f"- Exit code: {exit_code}",
                    f"- Attempted cases: {failure_summary['attempted_case_count']}/{failure_summary['declared_case_count']}",
                    f"- Materialized maps: {failure_summary['materialized_map_count']}/{failure_summary['declared_map_count']}",
                    f"- Candidate/resolved cells: {failure_summary['candidate_cell_count']}/{failure_summary['resolved_cell_count']}",
                    f"- Candidate/resolved paths: {failure_summary['candidate_path_count']}/{failure_summary['resolved_path_count']}",
                    f"- Candidate/resolved orbits: {failure_summary['candidate_orbit_count']}/{failure_summary['resolved_orbit_count']}",
                    "- Criterion numerically promoted: false",
                    "- Search v3/plasma coupling ready: false/false",
                    "",
                    "See `failure.json`, `phase-status.json`, `access-log.jsonl`,",
                    "per-case prerecords, map artifacts, GPU replay checkpoints,",
                    "and stdout/stderr logs for the exact partial evidence.",
                    "",
                    "The v1 first case is disclosed as accessed and is excluded from v2.",
                    "",
                )
            ),
            encoding="utf-8",
        ...
        summary = load_semantic_json(output / "dataset.json")["summary"]
        status = "success"
    inventory = [
        _inventory_entry(output, path)
        for path in sorted(output.rglob("*"))
...
def validate_results(output_dir: Path) -> dict[str, Any]:
    ...
    if (output / "failure.json").exists() and not (output / "dataset.json").exists():
        failure = load_semantic_json(output / "failure.json")
        lock = _strict_json(output / "execution-lock.json")
        if (
            manifest["status"] != "failed"
            or failure["status"] != "failed_immutable_no_patch_no_rerun"
            or lock["attempt"] != 1
            or lock["exit_code"] in (None, 0)
            or lock["status"] != "single_execution_failed_immutable"
