"""Steady-state v5: preregistered 25 um / 1.0 ps / W 1.5e4 ladder point - is the 33.3 um (v4) plateau itself resolution-converged?

The v4 refinement classified the accepted 50 um base plateau RESOLUTION-LIMITED (I_d +10.4 %, peak n_e -21.4 %, T_e,peak -24.5 %
at the predeclared 10 / 20 % tolerances, ``pic2d_cft_steady_state_v4/results/assessment.json``).  This experiment is the next
rung of the same ladder: the same channel, operating point, v1.3 closure and v2.0.3 gates on a grid refined by another 4/3 per
axis (dr = dz = 25 um, 120 x 960 cells, every geometry line on a grid line), dt 1.0 ps, macro-weight 1.5e4 (= 6e4 x (25/50)^2:
the base's particles per cell), assessed against the 33.3 um v4 plateau as its PRIMARY reference with the 50 um base reported
as the secondary (ladder) column.

Stages (from ``modern/``, ``$env:PYTHONPATH="$PWD\\src;$PWD"``)::

    python -m experiments.pic2d_cft_steady_state_v5.run preflight            # real field + mesh + memory + ms/step -> preflight.json
    python -m experiments.pic2d_cft_steady_state_v5.run shakedown            # short real-input run through finalize + assess -> shakedown.json
    python -m experiments.pic2d_cft_steady_state_v5.run launch [--expect-commit SHA] [--resume]   # clean worktree, exclusive lock, run
    python -m experiments.pic2d_cft_steady_state_v5.run status
    python -m experiments.pic2d_cft_steady_state_v5.run finalize [...]       # only for an externally stopped run (shared runner)
    python -m experiments.pic2d_cft_steady_state_v5.run assess               # predeclared acceptance (a)-(d) -> results/assessment.json

The stepping itself is the shared runner ``experiments.pic2d_cft_steady_state_v1.run``; the stages are the v4 preregistration
discipline (``pic2d_cft_steady_state_v4/run.py``) with this experiment's paths, reference and assessment schema - the v4 module is
frozen with its executed run and is not modified.  Detached launch (PowerShell, from ``modern/``)::
...
ASSESSMENT_SCHEMA = "cft-revival.pic2d-cft-steady-state-v5.assessment/1.0.0"
# primary reference: the 33.3 um v4 plateau; secondary (reported, not judged): the 50 um v2 base
REFERENCE_RESULTS = MODERN / "experiments" / "pic2d_cft_steady_state_v4" / "results"
SECONDARY_REFERENCE_RESULTS = MODERN / "experiments" / "pic2d_cft_steady_state_v2" / "results"
...
def gpu_load_snapshot(timeout_s: float = 5.0) -> dict[str, Any] | None:
    """nvidia-smi utilisation / memory BEFORE the preflight runs: a busy device (other CUDA processes) makes the timing an upper bound.

    None when nvidia-smi is unavailable or times out (never raises; nothing here is a result).
    """

    try:
        completed = subprocess.run(["nvidia-smi", "--query-gpu=utilization.gpu,memory.used,memory.total", "--format=csv,noheader,nounits"],
    ...
    try:
        return {"utilization_percent": float(fields[0]), "memory_used_mib": float(fields[1]), "memory_total_mib": float(fields[2]),
                "note": "sampled before this process touched the device; utilisation >> 0 means other processes share the GPU and the ms/step below is an upper bound"}
...
def preflight(protocol: dict[str, Any], *, backend: str = "warp-cuda", timing_steps: int = 2000, loaded_seed_density: float = 1.75e17,
    ...
    """Real inputs (P2 field on the 25 um grid, mesh, cross sections), factorisation, memory and ms/step at two loads.

    ``loaded_seed_density`` seeds a synthetic plasma at the expected plateau load (2 x 1.75e17 x 3.44e-7 / 1.5e4 = 8.0 M macro-particles;
    the v4 plateau held 1.54e17 mean -> 7.1 M at W 1.5e4) so the loaded step cost is measured, not extrapolated.  Non-evidentiary.
    """
    ...
    record: dict[str, Any] = {
        "schema_version": "cft-revival.pic2d-cft-steady-state-v5.preflight/1.0.0", "utc": utc_now(), "git_head": runner.git_head(),
    ...
    log(f"[preflight] grid {grid.cell_shape} dr {grid.dr_m*1e6:.3f} um dz {grid.dz_m*1e6:.3f} um; field {field_map.sha256[:12]} max |B| "
        ...
        f"seed {seed_state.electrons.count} e-; GPU before: {gpu_before}")
    ...
    record["projection"] = {
        ...
        "hours_to_3_transits_at_plateau_load": steps_3 * ms / 3.6e6, "hours_to_v4_verdict_time_at_plateau_load": steps_v4_verdict * ms / 3.6e6,
        ...
        "note": "the particle count grows from the seed (~2.3 M) to the plateau (~7-8 M) over the first ~1.5 us, so the wall time to 3 "
                "transits lies between the two projections, close to the plateau-load one; a preflight timed under GPU contention "
                "(other processes on the device) overstates the solo ms/step - the log records nvidia-smi's compute processes",
    }
    ref = protocol["reference_run"]["quantities"]
    ref2 = protocol["secondary_reference_run"]["quantities"]
    ...
    record["expected_at_v2_base_peak"] = _expected_gate_reading(max(grid.dr_m, grid.dz_m), config.dt_s, ref2["peak_n_e_window_per_m3"], ref2["t_e_peak_window_ev"], config)
...
def shakedown(protocol: dict[str, Any], *, results: Path | None = None, backend: str = "warp-cuda", output: Path = SHAKEDOWN_PATH,
    ...
    record = {
        "schema_version": "cft-revival.pic2d-cft-steady-state-v5.shakedown/1.0.0", "utc": utc_now(), "git_head": runner.git_head(),
        ...
        "assessment": {k: assessment[k] for k in ("verdict", "a_plateau", "b_residual_power", "c_convergence", "reference_consistency", "secondary_comparison")},
...
def reference_quantities_from_files(results: Path = REFERENCE_RESULTS) -> dict[str, float] | None:
    """Recompute the pinned reference numbers from a finished steady-state results directory (v4 primary / v2 secondary)."""
...
def _compare(run: dict[str, Any], reference: dict[str, float], tolerances: dict[str, float]) -> tuple[dict[str, Any], bool]:
    rows: dict[str, Any] = {}
    all_within = True
...
def assess(protocol: dict[str, Any], results: Path = RESULTS, *, output: Path | None = None, reference_check: bool = True,
           log: Callable[[str], None] = lambda text: print(text, flush=True)) -> dict[str, Any]:
    """Predeclared acceptance (protocol.stopping_rule.acceptance) against protocol.reference_run.quantities (the 33.3 um v4 plateau).

    The 50 um base (``protocol.secondary_reference_run``) is compared with the same tolerances and REPORTED (ladder column); it does
    ...
    secondary = protocol["secondary_reference_run"]["quantities"]
    ...
    convergence, all_within = _compare(run, reference, tolerances)
    secondary_rows, secondary_within = _compare(run, secondary, tolerances)
    ...
    if reference_check:
        ...
        secondary_consistency = _consistency(secondary, SECONDARY_REFERENCE_RESULTS)
        if secondary_consistency is not None and not all(entry["agree"] for entry in secondary_consistency.values()):
            raise PIC2DValidationError("protocol.secondary_reference_run.quantities disagree with the v2 artifacts on disk: " + json.dumps(
                {k: v for k, v in secondary_consistency.items() if not v["agree"]}))
    record = {
        ...
        "c_convergence": {"all_within": all_within, "quantities": convergence, "rule": acceptance["c_convergence_tolerances"]["note"],
                          "reference": "pic2d_cft_steady_state_v4/results (33.3 um, primary)"},
        "secondary_comparison": {"reference": "pic2d_cft_steady_state_v2/results (50 um base; reported, not judged)", "all_within": secondary_within,
                                 "quantities": secondary_rows, "reference_quantities": secondary, "reference_consistency": secondary_consistency,
                                 "v4_verdict_against_it": protocol["secondary_reference_run"]["v4_verdict"]},
    ...
    log(f"[assess] {results.name}: verdict {verdict} (a {a_plateau}, b {b_ok} [{windowed}], c all_within {all_within} vs 33 um); "
        + ", ".join(f"{k} {v['relative_difference']*100:+.1f}%" for k, v in convergence.items())
        + " | vs 50 um: " + ", ".join(f"{k} {v['relative_difference']*100:+.1f}%" for k, v in secondary_rows.items()))
    return record
