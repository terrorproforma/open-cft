...
SCHEMA_VERSION = "cft-revival.cft-wall-cusp-validation-v6.dataset/1.0.0"
...
def run_serialization_preflight() -> dict[str, Any]:
    ...
    return {
        "schema_version": "cft-revival.cft-wall-cusp-validation-v6.serialization-preflight/1.0.0",
        "status": "passed",
...
def run_production_path_static_preflight() -> dict[str, Any]:
    ...
    if len(policy_calls) != 2:
        raise ValueError("v6 must have two fully explicit MapValidationPolicy calls")
    ...
    if not reload_calls:
        raise ValueError("v6 production code must reload canonical field bytes")
    for call in reload_calls:
        ...
        if not (isinstance(legacy, ast.Constant) and legacy.value is False):
            raise ValueError("every v6 field reload must explicitly disable legacy v1.1")
    ...
    if policy != expected:
        raise ValueError("runtime map policy differs from the complete v6 policy")
    return {
        "status": "passed",
...
def dependency_closure() -> dict[str, Any]:
    ...
    paths = tuple(
        ...
        if (
            ...
            or path.startswith("modern/experiments/cft_wall_cusp_validation_v6/")
        )
...
def case_definitions() -> tuple[CaseDefinition, ...]:
    ...
    for stages, pitch_index, radius_index, polarity in itertools.product(
        ...
        case_id = (
            f"wcval-v6-s{int(stages):02d}-p{pitch_index}-r{radius_index}-{sign}"
        )
...
def _manufactured_development_case() -> BuiltCase:
    definition = CaseDefinition(
        case_id="development-manufactured-v6-field-pipeline",
        geometry_family_id="cft-topology-characterization-v1-cartesian-family",
        geometry_id="development-manufactured-v6-field-pipeline-geometry",
        stage_count=3,
        pitch_m=0.006,
...
def _manufactured_validation_registration(
    ...
    manifest_id = "manufactured-v6-non-held-out-registration"
    case_ids = (case.definition.case_id,)
    family_ids = ("manufactured-v6-registration-family",)
...
def run_production_field_pipeline_preflight(
    ...
    """Run the actual v6 v1.2 adapter path without held-out geometry access."""
    ...
    for role in ("primary", "refined", "enlarged"):
        context.before_expensive(
            f"v6-production-preflight-{role}",
            kind="solver",
            details={
                "role": role,
        ...
        reloaded = reload_field_artifact_bytes(
            stored,
            source=f"v6-production-preflight-{role}",
            allow_legacy_v1_1=False,
        )
...
class HeldOutArtifactAdapter:
    adapter_id = "experiments.cft-wall-cusp-validation-v6.held-out-artifact"
...
class ValidationCallbacks:
    ...
    def development(self, context: RunContext) -> Decision:
        freeze = {
            "schema_version": "cft-wall-cusp-v6.threshold-freeze/1.0.0",
