...
SCHEMA_VERSION = "cft-revival.cft-wall-cusp-validation-v5.dataset/1.0.0"
FOUNDATION_COMMIT = "231873d23fa242b15d0c085447a3d34ad55162a7"
...
def run_serialization_preflight() -> dict[str, Any]:
    ...
    return {
        "schema_version": "cft-revival.cft-wall-cusp-validation-v5.serialization-preflight/1.0.0",
        "status": "passed",
...
def run_production_path_static_preflight() -> dict[str, Any]:
    """Reject implicit policy defaults or legacy reloads in v5 production code."""
    ...
    if len(policy_calls) != 2:
        raise ValueError("v5 must have two fully explicit MapValidationPolicy calls")
    ...
    if not reload_calls:
        raise ValueError("v5 production code must reload canonical field bytes")
    for call in reload_calls:
        ...
        if not (isinstance(legacy, ast.Constant) and legacy.value is False):
            raise ValueError("every v5 field reload must explicitly disable legacy v1.1")
    policy = map_policy()
    ...
    if policy != expected:
        raise ValueError("runtime map policy differs from the complete v5 policy")
    return {
        "status": "passed",
...
def dependency_closure() -> dict[str, Any]:
    ...
    paths = tuple(
        ...
        if (
            ...
            or path.startswith("modern/experiments/cft_wall_cusp_validation_v5/")
        )
...
def case_definitions() -> tuple[CaseDefinition, ...]:
    ...
    for stages, pitch_index, radius_index, polarity in itertools.product(
        ...
        case_id = (
            f"wcval-v5-s{int(stages):02d}-p{pitch_index}-r{radius_index}-{sign}"
        )
...
def _manufactured_development_case() -> BuiltCase:
    definition = CaseDefinition(
        case_id="development-manufactured-v5-field-pipeline",
        geometry_family_id="cft-topology-characterization-v1-cartesian-family",
        geometry_id="development-manufactured-v5-field-pipeline-geometry",
        stage_count=3,
        pitch_m=0.006,
...
def _manufactured_validation_registration(
    ...
    manifest_id = "manufactured-v5-non-held-out-registration"
    case_ids = (case.definition.case_id,)
    family_ids = ("manufactured-v5-registration-family",)
...
def run_production_field_pipeline_preflight(
    ...
    """Run the actual v5 v1.2 adapter path without held-out geometry access."""
    ...
    for role in ("primary", "refined", "enlarged"):
        context.before_expensive(
            f"v5-production-preflight-{role}",
            kind="solver",
            details={
                "role": role,
        ...
        path = f"preflight/production-field-pipeline/{role}-field-v1.2.json"
        context.write_blob(path, item.artifact_bytes)
        stored = context.store.read_bytes(path)
        reloaded = reload_field_artifact_bytes(
            stored,
            source=f"v5-production-preflight-{role}",
            allow_legacy_v1_1=False,
        )
...
class MapGuidingCenterOrbitAdapter:
    """Exact-map-bound Boris orbit, magnetic-moment, energy, and pitch verifier."""
    ...
    def __init__(self, map_set: Any, registrations: tuple[CFTCellRegistration, ...]) -> None:
        ...
        self.orbit_code_hash = hashlib.sha256(
            b"cft-wall-cusp-map-boris-orbit-model-v4\0" + bytes.fromhex(source_hash)
        ).hexdigest()
        ...
        self.convergence_config_hash = semantic_hash(
            {
                "gyro_steps_per_period": declaration["gyro_steps_per_period"],
                "maximum_gyro_periods": declaration["maximum_gyro_periods"],
    ...
    def _boris_evolution(
        self,
        snapshot: Any,
        path: tuple[tuple[float, float], ...],
        sample: ElectronOrbitSample,
        direction: int,
        gyro_steps_per_period: int,
    ) -> dict[str, Any]:
        ...
        start_r, start_z = path[0]
        br0 = bilinear_sample(field, field.b_r_t, path[0])
        bz0 = bilinear_sample(field, field.b_z_t, path[0])
        b0 = math.hypot(br0, bz0)
        if b0 <= 0.0:
        ...
        position = (start_r, 0.0, start_z)
        velocity = (
            parallel_speed * br0 / b0,
            perpendicular_speed,
            parallel_speed * bz0 / b0,
        )
        mu0 = (
            self._ELECTRON_MASS_KG * perpendicular_speed**2 / (2.0 * b0)
        )
        gyro_period = (
            2.0 * math.pi * self._ELECTRON_MASS_KG
            / (self._ELEMENTARY_CHARGE_C * b0)
        )
        dt = gyro_period / gyro_steps_per_period
        transit_time = self._length(path) / max(abs(parallel_speed), 0.1 * speed)
        duration = min(
            transit_time,
            float(PROTOCOL["orbit_verification"]["maximum_gyro_periods"])
            * gyro_period,
        )
        step_count = max(2, math.ceil(duration / dt))
        maximum_mu_variation = 0.0
        maximum_energy_drift = 0.0
        maximum_pitch_change = 0.0
        mirror_detected = False
        completed_steps = 0
        for _ in range(step_count):
            radius = math.hypot(position[0], position[1])
            point = (radius, position[2])
            if not (
                field.r_m[0] <= radius <= field.r_m[-1]
                and field.z_m[0] <= position[2] <= field.z_m[-1]
            ):
                break
            ...
            magnetic = (br * radial_x, br * radial_y, bz)
            b_value = math.sqrt(sum(item * item for item in magnetic))
            if b_value <= 0.0:
                break
            factor = (
                -self._ELEMENTARY_CHARGE_C
                * dt
                / (2.0 * self._ELECTRON_MASS_KG)
            )
            t_vector = tuple(factor * item for item in magnetic)
            t_squared = sum(item * item for item in t_vector)
            s_vector = tuple(2.0 * item / (1.0 + t_squared) for item in t_vector)
            v_prime_delta = self._cross(velocity, t_vector)
            v_prime = tuple(
                velocity[index] + v_prime_delta[index] for index in range(3)
            )
            v_plus_delta = self._cross(v_prime, s_vector)
            velocity = tuple(
                velocity[index] + v_plus_delta[index] for index in range(3)
            )
            position = tuple(
                position[index] + velocity[index] * dt for index in range(3)
            )
            v_squared = sum(item * item for item in velocity)
            parallel = sum(
                velocity[index] * magnetic[index] for index in range(3)
            ) / b_value
            perpendicular_squared = max(0.0, v_squared - parallel * parallel)
            observed_mu = (
                self._ELECTRON_MASS_KG * perpendicular_squared / (2.0 * b_value)
            )
            observed_energy = 0.5 * self._ELECTRON_MASS_KG * v_squared
            observed_pitch = math.atan2(
                math.sqrt(perpendicular_squared), abs(parallel)
            )
            maximum_mu_variation = max(
                maximum_mu_variation,
                abs(observed_mu - mu0) / max(abs(mu0), 1e-300),
            )
            maximum_energy_drift = max(
                maximum_energy_drift,
                abs(observed_energy - energy_j) / max(abs(energy_j), 1e-300),
            )
            maximum_pitch_change = max(
                maximum_pitch_change,
                abs(observed_pitch - sample.pitch_angle_rad),
            )
            mirror_detected = mirror_detected or direction * parallel < 0.0
            completed_steps += 1
        return {
            "gyro_steps_per_period": gyro_steps_per_period,
            "requested_step_count": step_count,
            "completed_step_count": completed_steps,
            "completed": completed_steps == step_count,
            "timestep_s": dt,
            "gyro_period_s": gyro_period,
            "mu_initial_j_per_t": mu0,
            "maximum_mu_relative_variation": maximum_mu_variation,
            "maximum_energy_relative_drift": maximum_energy_drift,
            "maximum_pitch_angle_change_rad": maximum_pitch_change,
            "mirror_detected": mirror_detected,
            "terminal_position_xyz_m": position,
            "terminal_velocity_xyz_m_per_s": velocity,
            "b_start_t": b0,
        }
    ...
    def verify_orbit(
        ...
        position_difference = math.sqrt(
            math.fsum(
                (left - right) ** 2
                for left, right in zip(
                    coarse["terminal_position_xyz_m"],
                    fine["terminal_position_xyz_m"],
                    strict=True,
                )
            )
        ) / max(length, 1e-300)
        velocity_scale = math.sqrt(
            math.fsum(item * item for item in fine["terminal_velocity_xyz_m_per_s"])
        )
        velocity_difference = math.sqrt(
            math.fsum(
                (left - right) ** 2
                for left, right in zip(
                    coarse["terminal_velocity_xyz_m_per_s"],
                    fine["terminal_velocity_xyz_m_per_s"],
                    strict=True,
                )
            )
        ) / max(velocity_scale, 1e-300)
        state_difference = max(position_difference, velocity_difference)
        ...
        converged = bool(
            ...
            and all(item["completed"] for item in refinements)
        ...
        self.diagnostics[(path_hash, sample.sample_id)] = {
            ...
            "terminal_position_relative_difference": position_difference,
            "terminal_velocity_relative_difference": velocity_difference,
            "polyline_length_relative_defect": polyline_defect,
            "path_length_gate_passed": polyline_defect
...
def _record_summary(
    ...
    uncertainty_ordered = all(
        ...
        and 0.0 <= path.probability_lower
...
class HeldOutArtifactAdapter:
    adapter_id = "experiments.cft-wall-cusp-validation-v5.held-out-artifact"
...
def _report(dataset: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# Coupling v4.2 wall-cusp held-out validation v5",
        "",
...
class ValidationCallbacks:
    ...
    def prebundle(self, context: RunContext) -> Mapping[str, Any]:
        ...
        context.write_json("protocol/preregistered.json", PROTOCOL)
        context.write_json("provenance/dependency-closure.json", self.closure)
        context.write_json("provenance/runtime-identity.json", self.runtime_identity)
        context.write_json("preflight/serialization.json", preflight)
        context.write_json("preflight/production-path-static.json", static_preflight)
        context.write_json(
            "preflight/production-field-pipeline.json",
            production_preflight,
        )
        context.write_json("preflight/disjointness.json", disjointness)
        return {
    ...
    def development(self, context: RunContext) -> Decision:
        freeze = {
            "schema_version": "cft-wall-cusp-v5.threshold-freeze/1.0.0",
        ...
        context.write_json("development/threshold-freeze.json", freeze)
        return Decision(
            accepted,
            {
    ...
    def assessment(self, context: RunContext) -> Decision:
        ...
        for definition in case_definitions():
            ...
            context.write_json(f"replay/{definition.case_id}.json", replay)
            topology = {
        ...
        context.write_json("dataset.json", dataset)
        context.write_blob(
            "report.md",
            (_report(normalize(dataset)) + "\n").encode("utf-8"),
        )
        return Decision(
            promotion,
            {
...
def execute_validation(
    *,
    ...
    validate_bundle(result_root)
    return outcome


def validate_results(result_root: Path | None = None) -> Mapping[str, Any]:
    return validate_bundle(result_root or EXPERIMENT_DIR / "results")
