...
from experiments.cft_wall_cusp_validation_v3.experiment import (
...
from experiments.cft_wall_cusp_validation_v3.canonical import write_canonical, write_raw
from experiments.cft_wall_cusp_validation_v3.control import (
    attempt_payload,
...
def test_24_case_held_out_family_is_explicit_and_disjoint() -> None:
    ...
    assert {item.stage_count for item in definitions} == {3, 6, 9}
    assert {item.pitch_m for item in definitions} == {0.0055, 0.0072}
    assert {item.chamber_radius_m for item in definitions} == {0.0086, 0.0115}
    ...
    assert "accessed/non-held-out" in excluded["disclosure"]
    assert excluded["v2_accessed_case_ids"] == []
    assert not {
...
def test_end_to_end_canonical_serialization_matrix_and_map_orbits() -> None:
    ...
    assert preflight["prior_validation_held_out_map_access_count"] == 0
    ...
    assert not preflight["missing_dataclass_types"]
    assert set(preflight["declared_dataclass_types"]) <= set(
        preflight["serialized_dataclass_types"]
    )
    assert preflight["orbit_diagnostic_count"] > 0
    assert len(preflight["production_record_semantic_sha256"]) == 64


def test_every_actual_control_factory_uses_production_writer() -> None:
    report = run_control_preflight(
        protocol=PROTOCOL,
        protocol_semantic_sha256=PROTOCOL_SEMANTIC_SHA256,
        accepted_commit_sha=ACCEPTED_COUPLING_COMMIT,
    )
    assert report["status"] == "passed"
    assert report["held_out_access_count"] == 0
    names = {item["name"] for item in report["writer_cases"]}
    assert {
        "lock-acquired",
        "lock-finalized",
        "attempt-started",
    ...
    assert all(item["passed"] for item in report["failure_injections"])


def test_actual_failure_finalizer_and_manifest_are_preflighted(tmp_path: Path) -> None:
    ...
    acquired = write_canonical(
        ...
        lock_acquisition_payload(
            ...
            protocol_semantic_sha256=PROTOCOL_SEMANTIC_SHA256,
            command="manufactured-worker",
            declared_device=PROTOCOL["maps"]["solver"]["device"],
        ),
        exclusive=True,
    )
    acquired_bytes = (output / "execution-lock-acquired.canonical.json").read_bytes()
    write_canonical(
    ...
    write_canonical(
        ...
        {"status": "manufactured_pre_access_failure"},
        exclusive=True,
    )
    write_canonical(
    ...
    finalized = finalize_attempt(
        ...
        exit_code=1,
        stdout_bytes=b"",
        stderr_bytes=b"manufactured traceback\n",
        command="manufactured-worker",
    )
    assert finalized["status"] == "failed"
    assert (
        output / "execution-lock-acquired.canonical.json"
    ).read_bytes() == acquired_bytes
    assert validate_results(output)["status"] == "failed"
