"""Shakedown on real designs of both sets, gated preregistration, then execute exactly once.

Lifecycle (from ``modern/``)::

    python -m experiments.l1a_geometry_sweep_v3.run shakedown  # any HEAD, BEFORE prepare
    python -m experiments.l1a_geometry_sweep_v3.run prepare    # refuses without a valid shakedown.json
...
Ported from ``experiments.cusp_topology_search_v3_1.run`` (accepted template).
"""
...
from cft_revival.experiment_runtime import ExecutionAttestation, ExperimentRuntime, canonical_bytes, semantic_sha256, strict_json_loads, validate_bundle

from .designs import DESIGN_SETS
...
SUBJECT = "preregister L1a geometry sweep v3"
REMOTE_BRANCH = "origin/exp/l1a-geometry-sweep-v3"
COMMAND = "python -m experiments.l1a_geometry_sweep_v3.run execute"
SHAKEDOWN_COMMAND = "python -m experiments.l1a_geometry_sweep_v3.run shakedown"
...
EXPERIMENT_PREFIX = "modern/experiments/l1a_geometry_sweep_v3/"
TESTS_PREFIX = "modern/tests/experiments/l1a_geometry_sweep_v3/"
...
def _timing_projection(value: dict[str, Any], collector: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "basis": "mean shakedown seconds per design per set (solve N + solve 2N + QoIs + two characterizations + descriptors) times the evidentiary design counts, divided by the worker pool, times a contention factor",
...
def shakedown() -> dict[str, Any]:
    ...
    temp_root = Path(tempfile.gettempdir()) / f"l1a-gs-v3-shakedown-{git['head'][:12]}-{stamp}-{token}"
    ...
    record = {
        ...
        "disclosure": (
            "NON-EVIDENTIARY shakedown of the complete production path on REAL designs of both sets "
            "(Sobol designs at the extremes of the v3 box and the sweep-v2 held-out representatives). Its outcomes "
            "never enter any estimand; it exists only to prove the code path before the freeze. The descriptors are "
            "deterministic functions of the fields, so the shakedown designs' values are necessarily known before the "
            "freeze; the hypothesis, every threshold and every tolerance were fixed before the shakedown and are not "
            "tuned on it. The variable bounds were fixed after six corner feasibility solves for the sweep-v2 gates "
            "(disclosed in protocol.json#sampling.bounds_provenance), not on any rho value."
        ),
...
def prepare() -> dict[str, Any]:
    ...
    authorities = {
        ...
        "sobol_inside_sweep_v2_box_count": design_authorities["sobol_inside_sweep_v2_box_count"],
        "sobol_predicted_hemp_like_count": design_authorities["sobol_predicted_hemp_like_count"],
        "experiment_code_sha256": binding["experiment_code_sha256"],
        "dependency_source_sha256": binding["dependency_source_sha256"],
        "field_pipeline_source_sha256": binding["field_pipeline_source_sha256"],
        "sealed_sources": binding["sealed_sources"],
    ...
    return {
        ...
        "sobol_inside_sweep_v2_box_count": authorities["sobol_inside_sweep_v2_box_count"],
        "sobol_predicted_hemp_like_count": authorities["sobol_predicted_hemp_like_count"],
...
def execute() -> dict[str, Any]:
    ...
    value = protocol()
    cache_root = Path(tempfile.gettempdir()) / f"l1a-gs-v3-{commit[:12]}-working-cache"
    runtime = ExperimentRuntime(
