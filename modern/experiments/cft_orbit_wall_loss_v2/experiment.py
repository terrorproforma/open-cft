...
def runtime_launch_payload(launches: Sequence[Any]) -> dict[str, Any]:
    records = launch_records(launches)
    for record in records:
        record["seed_id"] = str(record["seed_id"])
    return {
        "schema_version": "cft-revival.cft-orbit-wall-loss-v2.launches/2.0.0",
...
def runtime_batch_payload(
    value: Mapping[str, Any], launches: Sequence[Any]
) -> dict[str, Any]:
    return {
        "schema_version": "cft-revival.cft-orbit-wall-loss-v2.batches/2.0.0",
        "estimator_policy": EstimatorPolicy.UNWEIGHTED_BINOMIAL.value,
...
def load_runtime_launch_payload(data: bytes) -> tuple[ElectronLaunch, ...]:
    ...
    if (
        ...
        != "cft-revival.cft-orbit-wall-loss-v2.launches/2.0.0"
...
def synthetic_serialization_audit(
    value: Mapping[str, Any],
    launches: Sequence[Any],
    batches: Sequence[Mapping[str, Any]],
) -> dict[str, Any]:
    """Round-trip every production vector shape without field/outcome access."""

    sample = launches[0]
    matrix = {
        "launch": runtime_launch_payload((sample,)),
        "final_state": {
    ...
    launch_bytes = canonical_bytes(runtime_launch_payload(launches))
    if load_runtime_launch_payload(launch_bytes) != tuple(
        sorted(launches, key=lambda item: item.launch_id)
    ):
        raise ValueError("typed launch payload roundtrip differs")
    batch_payload = runtime_batch_payload(value, launches)
    batch_bytes = canonical_bytes(batch_payload)
    if strict_json_loads(batch_bytes) != batch_payload:
        raise ValueError("batch payload roundtrip differs")
    ...
    checks = {
        ...
        "typed_launch_roundtrip": True,
        "batch_roundtrip": True,
    ...
    return {
        "schema_version": "cft-revival.cft-orbit-wall-loss-v2.serialization-audit/2.0.0",
        "covered_fields": [
        ...
        "launch_payload_byte_sha256": hashlib.sha256(launch_bytes).hexdigest(),
        "batch_payload_byte_sha256": hashlib.sha256(batch_bytes).hexdigest(),
        "checks": checks,
...
def callbacks() -> RuntimeCallbacks:
    ...
    frozen_launch_bytes = LAUNCH_MANIFEST_PATH.read_bytes()
    frozen_batch_bytes = BATCH_MANIFEST_PATH.read_bytes()
    synthetic = strict_json_file(SYNTHETIC_PREFLIGHT_PATH)
    launches = build_launches(value)
    batches = batch_records(value, launches)
    expected_launch_bytes = canonical_bytes(runtime_launch_payload(launches))
    expected_batch_bytes = canonical_bytes(runtime_batch_payload(value, launches))
    state: dict[str, Any] = {}
    ...
    def prebundle(context: Any) -> Mapping[str, Any]:
        ...
        if (
            frozen_launch_bytes != expected_launch_bytes
            or hashlib.sha256(frozen_launch_bytes).hexdigest()
            != authorities["runtime_launch_payload_byte_sha256"]
        ):
            raise ValueError("launch payload bytes differ from preregistered authority")
        if load_runtime_launch_payload(frozen_launch_bytes) != tuple(
            sorted(launches, key=lambda item: item.launch_id)
        ):
            raise ValueError("typed launch authority reconstruction differs")
        if (
            frozen_batch_bytes != expected_batch_bytes
            or hashlib.sha256(frozen_batch_bytes).hexdigest()
            != authorities["runtime_batch_payload_byte_sha256"]
        ):
            raise ValueError("batch payload bytes differ from preregistered authority")
        if (
            content_hash(launch_records(launches))
            != authorities["orbit_launches_sha256"]
            or content_hash(
                {
                    "estimator_policy": EstimatorPolicy.UNWEIGHTED_BINOMIAL.value,
                    "batches": batches,
            ...
            != authorities["batch_manifest_sha256"]
            or estimator_identity(launches, batches)
            != authorities["estimator_sha256"]
        ):
            raise ValueError("launch/batch/estimator external authority differs")
        ...
        context.write_blob("artifacts/launch-manifest.json", frozen_launch_bytes)
        context.write_blob("artifacts/batch-manifest.json", frozen_batch_bytes)
