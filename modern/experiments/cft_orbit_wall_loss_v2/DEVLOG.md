...
## 2026-09-02 — V2 preregistration

- Created isolated campaign authority on
  `exp/cft-orbit-wall-loss-v2` from `25dbeaaff4d3a3c276866fdc3f461302a0631120`.
...
- Disclosed that v1 stopped during prebundle with zero P2 and orbit access;
  retained its launch grid but recomputed v2 campaign, launch and seed IDs.
- Replaced parsed tagged-payload comparison with one canonical launch-payload
  builder, exact byte/SHA-256 authority checks, and closed typed reconstruction.
- Added synthetic coverage for every production tuple/list/vector family and
  explicit malformed/reserved/double-tag encoding rejection.
- Validation before freezing the preregistration:
  - campaign protocol: 6 passed;
  - accepted orbit v1.4: 94 passed;
  - shared runtime: 132 passed, 1 Windows privilege skip;
  - production synthetic preflight: passed with 512 launches, 8 batches,
    zero P2 field access and zero orbit outcome access.
