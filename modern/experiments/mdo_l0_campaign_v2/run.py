"""Shakedown on the real catalogue and L0 model, gated preregistration, then execute exactly once.

Lifecycle (from ``modern/`` with the ``.venv-sota`` interpreter)::

    python -m experiments.mdo_l0_campaign_v2.run shakedown   # any HEAD, dirty ok, BEFORE prepare
    python -m experiments.mdo_l0_campaign_v2.run prepare     # refuses without a valid shakedown.json
    # commit + push the preregistration, then from a clean detached worktree:
    python -m experiments.mdo_l0_campaign_v2.run execute
    python -m experiments.mdo_l0_campaign_v2.run validate
"""
...
SUBJECT = "preregister MDO L0 campaign v2"
REMOTE_BRANCH = "origin/exp/mdo-l0-campaign-v2"
COMMAND = "python -m experiments.mdo_l0_campaign_v2.run execute"
SHAKEDOWN_COMMAND = "python -m experiments.mdo_l0_campaign_v2.run shakedown"
DEVICE = "cpu-l0-python-reference+cpu-gp-float64"
FROZEN_OUTPUTS = (AUTHORITIES_PATH,)
EXPERIMENT_PREFIX = "modern/experiments/mdo_l0_campaign_v2/"
TESTS_PREFIX = "modern/tests/experiments/mdo_l0_campaign_v2/"
...
def shakedown() -> dict[str, Any]:
    ...
    temp_root = Path(tempfile.gettempdir()) / f"mdo-l0-campaign-v2-shakedown-{git['head'][:12]}-{stamp}-{token}"
    ...
    record = {
        ...
        "disclosure": (
            "NON-EVIDENTIARY shakedown of the complete production path on the real screened catalogue "
            "and the real L0 model with a small budget and a seed namespace disjoint from the evidentiary "
            "campaign. Its outcomes never enter any estimand; it exists only to prove the code path before the freeze."
        ),
        "attestation_note": (
            "ExecutionAttestation.clean_worktree is a runtime contract constant; the actual git dirtiness at "
            "shakedown time is recorded under git.dirty_entries"
        ),
    ...
    return {
        ...
        "bundle_error": bundle_error,
        "result_root": str(result_root),
        "import_scope_matches": scope["matches"],
        ...
        "binding_gates": {name: item["passed"] for name, item in (gates.get("binding") or {}).items()},
        "reported": {
...
def execute() -> dict[str, Any]:
    ...
    value = protocol()
    cache_root = Path(tempfile.gettempdir()) / f"mdo-l0-campaign-v2-{commit[:12]}-working-cache"
    runtime = ExperimentRuntime(
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("command", choices=("shakedown", "prepare", "execute", "validate", "contract", "imports"))
    ...
    if arguments.command == "shakedown":
        ...
        output = import_scope_report(protocol())
    else:
